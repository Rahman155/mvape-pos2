 ...
 */
export async function calculateInventoryValue(storeId: UUID): Promise<number> {
  if (!storeId) {
  ...
  try {
    ...
    logger.error('Failed to calculate inventory value', error as Error);
    throw new ApiError(
      `Failed to calculate inventory value for store ${storeId}`,
 ...
 */
export async function getCashInRegister(storeId: UUID): Promise<number> {
  if (!storeId) {
    throw new Error('Store ID is required');
  }
 ...
 */
export async function calculateStoreCapital(storeId: UUID): Promise<StoreCapital> {
  if (!storeId) {
    throw new Error('Store ID is required');
  }
  ...
  try {
    ...
    logger.error('Failed to calculate store capital', error as Error);
    throw new ApiError(
      `Failed to calculate capital for store ${storeId}`,
      ApiErrorCode.INTERNAL_ERROR
    );
  }
}
 ...
 */
export async function calculateTotalCapital(): Promise<{
  ...
  try {
    ...
    logger.error('Failed to calculate total capital', error as Error);
    throw new ApiError(
      'Failed to calculate total capital',
      ApiErrorCode.INTERNAL_ERROR
    );
  }
}
 ...
 * Requirement: 22.3
 */
export async function calculateMultiStoreCapital(
  storeIds: UUID[]
): Promise<Map<UUID, StoreCapital>> {
  if (!Array.isArray(storeIds) || storeIds.length === 0) {
    throw new Error('At least one store ID is required');
  }

  const results = new Map<UUID, StoreCapital>();
