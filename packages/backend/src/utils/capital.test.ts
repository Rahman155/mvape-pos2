...
import { calculateStoreCapital, calculateInventoryValue } from './capital.js';
...
describe('Capital Calculation - Property-Based Tests', () => {
  ...
  it('Property 4: Capital should never be negative with valid inputs', () => {
    ...
    const inventoryItemsArbitrary = fc.array(
      fc.tuple(quantityArbitrary, costPriceArbitrary),
      { minLength: 0, maxLength: 20 }
    );

    fc.assert(
      fc.property(
        inventoryItemsArbitrary,
        cashArbitrary,
        (inventoryItems, cash) => {
  ...
  it('Property 4c: Capital non-negativity with edge case inventory configurations', () => {
    const quantityArbitrary = fc.oneof(
      fc.constant(0), // Empty inventory
      fc.integer({ min: 1, max: 100 }),
      fc.integer({ min: 100, max: 10000 }) // Large quantities
    );
    
    const costPriceArbitrary = fc.oneof(
      fc.constant(0), // Free items
      fc.integer({ min: 1, max: 10000 }),
      fc.integer({ min: 10000, max: 1000000 }) // High-value items
    );
    
    const cashArbitrary = fc.oneof(
      fc.constant(0), // No cash
      fc.integer({ min: 1, max: 1000000 }),
      fc.integer({ min: 1000000, max: 100000000 }) // Large cash amounts
    );
  ...
  it('Property 4d: Capital non-negativity with realistic vape store inventory', () => {
    ...
    const inventoryArbitrary = fc.record({
      eLiquids: fc.array(
        fc.tuple(stockLevel, eLiquidPrice),
        { maxLength: 50 }
      ),
      mods: fc.array(
        fc.tuple(stockLevel, modPrice),
        { maxLength: 30 }
      ),
      coils: fc.array(
        fc.tuple(stockLevel, coilPrice),
        { maxLength: 100 }
      ),
      accessories: fc.array(
        fc.tuple(stockLevel, accessoryPrice),
        { maxLength: 200 }
      ),
    });
    ...
    fc.assert(
      fc.property(
        ...
        (inventory, cash) => {
          ...
          for (const category of ['eLiquids', 'mods', 'coils', 'accessories'] as const) {
  ...
  it('Property 4e: Capital conservation - combining stores maintains non-negativity', () => {
    ...
    fc.assert(
      fc.property(
        ...
        cashArbitrary,
        cashArbitrary,
        (store1Inventory, store2Inventory, store1Cash, store2Cash) => {
