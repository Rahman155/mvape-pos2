 ...
 * Requirement: 6.2, 6.3
 */
export async function calculateDailyStatistics(
  storeId: string,
  startDate: Date,
  endDate: Date,
  includeStatus: string = 'COMPLETED'
  ...
  try {
    ...
    const row = result.rows[0] as TransactionData;
    ...
    throw new Error(
      `Failed to calculate daily statistics: ${error instanceof Error ? error.message : 'Unknown error'}`
    );
  }
}
 ...
 * Requirement: 6.4, 9.4
 */
export async function getActiveBop(storeId: string): Promise<BopData | null> {
  if (!storeId) {
    throw new Error('Store ID is required');
  }
  ...
  try {
    ...
    throw new Error(
      `Failed to fetch active BOP: ${error instanceof Error ? error.message : 'Unknown error'}`
    );
  }
}
 ...
 */
export async function calculateMultiStoreStatistics(
  storeIds: string[],
  startDate: Date,
  endDate: Date
  ...
  const results = new Map<string, DailyStatistics>();
...
export async function calculateAggregatedStatistics(
  ...
  try {
    ...
    throw new Error(
      `Failed to calculate aggregated statistics: ${error instanceof Error ? error.message : 'Unknown error'}`
    );
  }
}
 ...
 */
export function roundToPrecision(value: number, decimals: number = 0): number {
