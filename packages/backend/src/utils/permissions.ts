 ...
 */
export const PERMISSIONS: FeaturePermissions = {
  AUTH: {
    LOGIN: 'public',
    LOGOUT: ['OWNER', 'KASIR'],
 ...
 */
export function hasRole(userRole: Role, requiredRoles: Permission): boolean {
 ...
 */
export function getFeaturePermission(feature: keyof typeof PERMISSIONS, action: string): Permission | null {
  const featurePerms = PERMISSIONS[feature];
  if (!featurePerms) return null;

  const actionKey = action.toUpperCase() as any;
