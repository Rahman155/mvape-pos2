...
describe('Stock Transfer Endpoints', () => {
  ...
  beforeAll(async () => {
    ...
    await db.query(
      ...
      [warehouseStoreId, 'WAREHOUSE', 'Warehouse Address']
    );
    ...
    await db.query(
      ...
      [testOwnerId, 'testowner', `owner_${Date.now()}@test.com`, hashedPassword, 'OWNER', warehouseStoreId, true]
    );
    ...
    // Create test product
    await db.query(
      `INSERT INTO products (id, name, sku, cost_price, selling_price, is_active, created_at, updated_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
       ON CONFLICT DO NOTHING`,
      [testProductId, 'Test Product', 'SKU001', '10000', '15000', true, new Date(), new Date()]
    );
    ...
    await db.query(
      ...
      [uuidv4(), testProductId, warehouseStoreId, 100, 0, 10, new Date(), new Date()]
    );
  ...
  describe('POST /api/stock-transfers', () => {
    ...
    it('should deduct from warehouse inventory', async () => {
      ...
      const beforeResult = await db.query(
        ...
        [testProductId, (await db.query("SELECT id FROM stores WHERE name = 'WAREHOUSE' LIMIT 1")).rows[0].id]
      );
  ...
  describe('GET /api/stock-transfers', () => {
    ...
    it('should filter by status', async () => {
      ...
      expect(response.status).toBe(200);
      response.body.data.forEach((transfer: any) => {
