...
describe('BOP Endpoints', () => {
  ...
  beforeAll(async () => {
    ...
    await db.query(
      ...
      [testStoreId, 'Test Store 1', 'Address 1', true]
    );

    await db.query(
      'INSERT INTO stores (id, name, address, is_active) VALUES ($1, $2, $3, $4) ON CONFLICT DO NOTHING',
      [store2Id, 'Test Store 2', 'Address 2', true]
    );
    ...
    await db.query(
      ...
      [testUserId, 'testuser', `user_${Date.now()}@test.com`, hashedPassword, 'OWNER', testStoreId, true]
    );
    ...
    await db.query(
      `INSERT INTO bop (id, store_id, name, description, amount, effective_from, effective_to, created_at, updated_at)
       ...
       ON CONFLICT DO NOTHING`,
      [bopId1, testStoreId, 'Electricity', 'Monthly electricity cost', '500000', today, null, new Date(), new Date()]
    );
    ...
    await db.query(
      `INSERT INTO bop (id, store_id, name, description, amount, effective_from, effective_to, created_at, updated_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
       ON CONFLICT DO NOTHING`,
      [bopId2, store2Id, 'Rent', 'Monthly rent', '10000000', today, null, new Date(), new Date()]
    );
  ...
  describe('GET /api/bop', () => {
    ...
    it('should filter by store ID', async () => {
      ...
      response.body.data.forEach((bop: any) => {
