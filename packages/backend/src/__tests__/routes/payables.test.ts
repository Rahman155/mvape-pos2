...
describe('Payables Management', () => {
  ...
  beforeAll(async () => {
    ...
    await db.query(
      `INSERT INTO piutang (id, supplier_id, amount, remaining_balance, due_date, status, created_at, updated_at)
  ...
  describe('GET /api/payables', () => {
    ...
    it('should support filtering by status', async () => {
      ...
      expect(response.status).toBe(200);
      response.body.data.forEach((payable: any) => {
