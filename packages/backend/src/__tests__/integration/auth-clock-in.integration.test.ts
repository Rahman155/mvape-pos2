...
describe('Clock-In on Login Integration Tests (Task 46 / Requirement 15.1)', () => {
  ...
  let kasirEmail: string;
  let kasirPassword: string;
  ...
  beforeAll(async () => {
    ...
    await db.query(
      ...
      [testStoreId, 'Test Store for Clock-In', '123 Test Street']
    );
    ...
    await db.query(
      ...
      [kasirUserId, 'kasir_test', kasirEmail, kasirPasswordHash, 'KASIR', testStoreId, true]
    );
    ...
    await db.query(
      `INSERT INTO users (id, username, email, password_hash, role, store_id, is_active) 
       VALUES ($1, $2, $3, $4, $5, $6, $7) 
       ON CONFLICT DO NOTHING`,
      [ownerUserId, 'owner_test', ownerEmail, ownerPasswordHash, 'OWNER', testStoreId, true]
    );
  ...
  describe('POST /api/v1/auth/login - Clock-in for KASIR users', () => {
    it('should record clock-in timestamp when KASIR user logs in', async () => {
      ...
      const result = await db.query(
        'SELECT id, user_id, clock_in, date, status FROM attendance WHERE id = $1',
        [attendanceId]
      );
  ...
  describe('Edge Cases and Constraints', () => {
    ...
    it('should handle incomplete logout detection with consecutive logins', async () => {
      ...
      await db.query(
        ...
        [uuidv4(), kasirUserId, clockInYesterday.toISOString(), yesterdayDateStr, 'PRESENT']
      );

      // Verify yesterday's record has no clock_out
      const resultBefore = await db.query(
        'SELECT clock_out FROM attendance WHERE user_id = $1 AND date = $2',
        [kasirUserId, yesterdayDateStr]
      );
      ...
      const resultAfter = await db.query(
        'SELECT clock_out, duration_minutes FROM attendance WHERE user_id = $1 AND date = $2',
        [kasirUserId, yesterdayDateStr]
      );
