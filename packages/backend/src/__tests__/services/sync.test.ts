...
import { processBatchSync, type SyncRequestItem } from '../../services/sync.js';
...
describe('Sync Service - Batch Processing', () => {
  ...
  beforeAll(async () => {
    ...
    await db.query(
      'INSERT INTO users (id, username, password_hash, role, store_id) VALUES ($1, $2, $3, $4, $5)',
      [testKasirId, 'testkasir', 'hash', 'KASIR', testStoreId]
    );
    ...
    // Insert test member
    await db.query(
      'INSERT INTO members (id, member_number, name, phone, credit_balance) VALUES ($1, $2, $3, $4, $5)',
      [testMemberId, 'MBR-001', 'Test Member', '08123456789', 100000]
    );
  });
  ...
  describe('Empty and Invalid Batches', () => {
    ...
    it('should handle null items gracefully', async () => {
      const response = await processBatchSync(null as any);
  ...
  describe('Transaction Sync', () => {
    ...
    it('should create transaction and deduct inventory', async () => {
      ...
      const initialInventory = await db.query(
