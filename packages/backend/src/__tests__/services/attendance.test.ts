...
import { AttendanceService, type AttendanceRecord } from '../../services/attendance.js';
...
describe('Attendance Service', () => {
  ...
  describe('clockOut', () => {
    ...
    it('should throw error if no clock-in record found', async () => {
      ...
      await expect(
        AttendanceService.clockOut(testUserId, clockOutTime)
      ).rejects.toThrow('No attendance record found');
    });
  ...
  describe('getAttendanceByPeriod', () => {
    ...
    it('should return attendance records for period', async () => {
      ...
      const { data, total } = await AttendanceService.getAttendanceByPeriod(
        testUserId,
        startDate,
        endDate
      );
    ...
    it('should support pagination', async () => {
      ...
      const { data: page1 } = await AttendanceService.getAttendanceByPeriod(
        testUserId,
        startDate,
        endDate,
        2,
      ...
      const { data: page2 } = await AttendanceService.getAttendanceByPeriod(
        testUserId,
        startDate,
        endDate,
        2,
    ...
    it('should include user details in attendance records', async () => {
      ...
      const { data } = await AttendanceService.getAttendanceByPeriod(
        testUserId,
        startDate,
        endDate
      );
  ...
  describe('getMonthlyReport', () => {
    ...
    beforeEach(async () => {
      ...
      for (let i = 1; i <= 20; i++) {
        ...
        await db.query(
          ...
          [uuidv4(), kasir1Id, clockIn, clockOut, 480, '2024-01-' + String(i).padStart(2, '0'), 'PRESENT']
        );
      }
      ...
      for (let i = 1; i <= 15; i++) {
        ...
        await db.query(
          `INSERT INTO attendance (id, user_id, clock_in, clock_out, duration_minutes, date, status, created_at)
           VALUES ($1, $2, $3, $4, $5, $6, $7, NOW())`,
          [uuidv4(), kasir2Id, clockIn, clockOut, 480, '2024-01-' + String(i).padStart(2, '0'), 'PRESENT']
        );
      }
    });
    ...
    it('should throw error for invalid month', async () => {
      await expect(
        AttendanceService.getMonthlyReport(2024, 13)
      ).rejects.toThrow('Invalid month');

      await expect(
        AttendanceService.getMonthlyReport(2024, 0)
      ).rejects.toThrow('Invalid month');
    });
