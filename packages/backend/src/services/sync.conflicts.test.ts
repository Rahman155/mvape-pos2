...
import { processBatchSync, type SyncRequestItem, type ConflictDetectionResult } from './sync';
...
describe('Server-Side Conflict Detection', () => {
  ...
  describe('Conflict Resolution Application', () => {
    it('should apply LWW resolution strategy', async () => {
      ...
      vi.mocked(db.transaction).mockImplementation(async (callback) => {
        return callback({ query: vi.fn() } as any);
      });
  ...
  describe('Multiple Conflicts Handling', () => {
    ...
    it('should track conflict statistics', async () => {
      ...
      // Mock for 3 entities
      vi.mocked(db.query)
        .mockResolvedValueOnce({ rows: [{ updated_at: new Date(now + 1000) }] } as any)
        .mockResolvedValueOnce({ rows: [{ updated_at: new Date(now + 1000) }] } as any)
        .mockResolvedValueOnce({ rows: [{ updated_at: new Date(now - 1000) }] } as any);
  ...
  describe('Conflict Idempotency', () => {
    it('should handle repeated conflict detection idempotently', async () => {
      ...
      expect(
        result1.results[0].conflict?.detected ===
          result2.results[0].conflict?.detected
      ).toBe(true);
    });
  });
});
