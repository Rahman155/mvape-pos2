...
import { Inventory, StockTransfer, StockTransferItem, StockTransferWithItems } from '../database/types.js';
...
export class InventoryService {
   ...
   */
  static async getTotalInventoryQuantity(productId: UUID): Promise<number> {
    try {
      ...
      logger.error('Failed to get total inventory quantity', error as Error);
      throw new ApiError(
        'Failed to get total inventory quantity',
   ...
   */
  static async getInventory(productId: UUID, storeId: UUID): Promise<Inventory | null> {
    try {
      const result = await db.query(
      ...
      logger.error('Failed to get inventory', error as Error);
  ...
  static async createStockTransfer(
    ...
    try {
      ...
      for (const item of request.items) {
        const sourceInventory = await this.getInventoryForUpdate(
          item.productId,
        ...
        if (!sourceInventory) {
          throw new ApiError(
            `No inventory found for product ${item.productId} at source location`,
        ...
        if (availableQuantity < item.quantity) {
          throw new ApiError(
            `Insufficient inventory for product ${item.productId}. Available: ${availableQuantity}, Requested: ${item.quantity}`,
      ...
      for (const item of request.items) {
        // Decrease at source
        await client.query(
          'UPDATE inventory SET quantity = quantity - $1, updated_at = NOW() WHERE product_id = $2 AND store_id = $3',
          [item.quantity, item.productId, request.fromLocationId]
        );

        // Increase at destination
        const destInventory = await this.getInventoryForUpdate(
          item.productId,
          request.toStoreId,
          client
        );
        ...
        if (!destInventory) {
          ...
          await client.query(
            `INSERT INTO inventory (id, product_id, store_id, quantity, reserved, reorder_level, created_at, updated_at)
             VALUES (gen_random_uuid(), $1, $2, $3, 0, 10, NOW(), NOW())`,
            [item.productId, request.toStoreId, item.quantity]
          );
        } else {
          await client.query(
            'UPDATE inventory SET quantity = quantity + $1, updated_at = NOW() WHERE product_id = $2 AND store_id = $3',
            [item.quantity, item.productId, request.toStoreId]
          );
        }
      }
      ...
      logger.error('Stock transfer creation failed', error as Error);
      throw error;
    } finally {
      client.release();
    }
   ...
   */
  private static async getInventoryForUpdate(
    productId: UUID,
    storeId: UUID,
    client: any
  ): Promise<Inventory | null> {
    const result = await client.query(
      'SELECT * FROM inventory WHERE product_id = $1 AND store_id = $2 FOR UPDATE',
      [productId, storeId]
    );
    return result.rows.length > 0 ? result.rows[0] : null;
  }
   ...
   */
  private static async createTransferRecord(
    request: StockTransferRequest,
    client: any
  ): Promise<UUID> {
    const result = await client.query(
      `INSERT INTO stock_transfers (id, from_location_id, to_store_id, transfer_date, status, created_by, created_at, updated_at)
       VALUES (gen_random_uuid(), $1, $2, NOW(), $3, $4, NOW(), NOW())
       RETURNING id`,
      [request.fromLocationId, request.toStoreId, 'PENDING', request.createdBy]
    );
   ...
   */
  private static async createTransferItems(
    transferId: UUID,
    items: Array<{ productId: UUID; quantity: number }>,
    client: any
  ): Promise<StockTransferItem[]> {
    const createdItems: StockTransferItem[] = [];

    for (const item of items) {
      const result = await client.query(
        `INSERT INTO stock_transfer_items (id, stock_transfer_id, product_id, quantity, received_quantity, created_at)
   ...
   */
  static async getStoreInventory(storeId: UUID): Promise<Inventory[]> {
    try {
      const result = await db.query(
      ...
      logger.error('Failed to get store inventory', error as Error);
