...
describe('Transaction Service', () => {
  ...
  beforeAll(async () => {
    ...
    await db.query(
      `INSERT INTO stores 
       ...
       VALUES ($1, $2, $3, $4, $5, $6)`,
      [testStoreId, 'Test Store', true, now, now, '{}']
    );
    ...
    await db.query(
      `INSERT INTO inventory 
       ...
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
      [uuidv4(), testProductId, testStoreId, 100, 0, 10, now, now]
    );
  });
  ...
  describe('deductMemberCredit', () => {
    beforeEach(async () => {
      ...
      await db.query(
        'UPDATE members SET credit_balance = $1 WHERE id = $2',
        ['5000000', testMemberId]
      );
    });
  ...
  describe('Member Credit Payment - Property-Based Tests', () => {
    it('should maintain credit balance conservation on successful deduction', async () => {
      ...
      const beforeResult = await db.query(
        'SELECT SUM(CAST(credit_balance AS DECIMAL)) as total FROM members'
      );
    ...
    it('should handle boundary condition: exact amount equals balance', async () => {
      ...
      await db.query(
        'UPDATE members SET credit_balance = $1 WHERE id = $2',
        [testAmount.toString(), testMemberId]
      );
