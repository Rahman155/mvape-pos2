...
export class ConflictNotificationManager {
  ...
  private subscribers: Set<(notification: ConflictNotification) => void> = new Set();
  ...
  notifyConflict(
    ...
    strategy: 'LWW' | 'MERGE' | 'MANUAL',
    reason: string,
    requiresReview: boolean = strategy === 'MANUAL'
    ...
    logger.info(
      `Conflict notification created: ${entityType} ${entityId} (${strategy}, requires review: ${requiresReview})`
    );
  ...
  resolveNotification(notificationId: string, resolution: any): void {
    ...
    logger.info(
      `Conflict notification resolved: ${notification.entityType} ${notification.entityId}`
    );
  }
...
// Singleton instance
let instance: ConflictNotificationManager | null = null;

/**
 * Get singleton instance of ConflictNotificationManager
 */
export function getConflictNotificationManager(): ConflictNotificationManager {
  if (!instance) {
