...
import { authorize, requireAuth, allowPublic } from '../middleware/authorize.js';

export const authRouter = express.Router() as ReturnType<typeof express.Router>;
 ...
 * }
 */
authRouter.post('/login', allowPublic(), async (req: Request, res: Response) => {
  try {
 ...
 * }
 */
authRouter.get('/me', authorize(['OWNER', 'KASIR']), async (req: Request, res: Response) => {
  try {
...
authRouter.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      logger.error('Get sessions error', error as Error, {
        userId: req.user?.id,
        requestId: req.requestId,
      });
      res.status(500).json({
...
authRouter.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      logger.error('Get login history error', error as Error, {
        userId: req.user?.id,
        requestId: req.requestId,
      });
      res.status(500).json({
