...
router.post('/', requireAuth(), async (req: Request, res: Response) => {
  try {
    ...
    if (paymentMethod === 'MEMBER_CREDIT') {
      ...
      const memberValidation = await validateMemberCredit(
        paymentData.memberCredit.memberId,
        paymentData.memberCredit.usedCredit
      );
...
router.get('/', requireAuth(), async (req: Request, res: Response) => {
  try {
    ...
    if (paymentMethods) {
      ...
      const methods = (paymentMethods as string).split(',').map(m => m.trim());
    ...
    const countResult = await db.query(
      query.replace('SELECT t.*', 'SELECT COUNT(*) as count'),
      params
    );
...
router.get('/:id', requireAuth(), async (req: Request, res: Response) => {
  try {
    ...
    const txn: any = txnResult.rows[0];
    ...
    try {
      ...
      await db.query(
        `UPDATE transactions 
         ...
         WHERE id = $6`,
        [totalAmount.toString(), notes || null, now, user.id, now, id]
      );
