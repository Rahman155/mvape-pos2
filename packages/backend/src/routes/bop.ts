...
router.get('/', requireAuth(), async (req: Request, res: Response) => {
  try {
    ...
    const bopList = result.rows.map((row: any) => ({
      id: row.id,
...
router.get('/store/:storeId', requireAuth(), async (req: Request, res: Response) => {
  try {
    ...
    const countResult = await db.query(
      'SELECT COUNT(*) FROM bop WHERE store_id = $1',
      [storeId]
    );
    const total = Number(countResult.rows[0].count);
...
router.post('/', authorize('OWNER'), async (req: Request, res: Response) => {
  try {
    ...
    const newBopResult = await db.query(
      `SELECT 
        b.id,
        b.store_id,
        b.name,
    ...
    logger.error('Error creating BOP:', error as Error, {
      requestId: req.requestId,
      userId: req.user?.id,
    });
...
router.put('/:id', authorize('OWNER'), async (req: Request, res: Response) => {
  try {
    ...
    const bopResult = await db.query(
      'SELECT id, store_id FROM bop WHERE id = $1',
      [id]
    );
    ...
    const updatedBopResult = await db.query(
      `SELECT 
        b.id,
        b.store_id,
        b.name,
    ...
    logger.error('Error updating BOP:', error as Error, {
      requestId: req.requestId,
      userId: req.user?.id,
      bopId: req.params.id,
    });
...
router.delete('/:id', authorize('OWNER'), async (req: Request, res: Response) => {
  try {
    ...
    // Verify BOP exists
    const bopResult = await db.query(
      'SELECT id, effective_from, effective_to FROM bop WHERE id = $1',
      [id]
    );
    ...
    logger.error('Error deleting BOP:', error as Error, {
      requestId: req.requestId,
      userId: req.user?.id,
      bopId: req.params.id,
    });
