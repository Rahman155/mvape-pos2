...
describe('Piutang Routes (Requirements 18.3-18.8)', () => {
  ...
  beforeEach(async () => {
    ...
    // Insert member
    await db.query(
      `INSERT INTO members (id, member_number, name, phone, email, credit_balance, total_spent)
       VALUES ($1, $2, $3, $4, $5, $6, $7)`,
      [memberId, 'M001', 'Test Customer', '08123456789', 'customer@test.com', 0, 0]
    );
  ...
  describe('GET /api/piutang/alerts/upcoming - Upcoming Piutang Alerts (Requirement 18.8)', () => {
    beforeEach(async () => {
      ...
      await db.query(
        `INSERT INTO piutang (id, transaction_id, member_id, amount, remaining_balance, due_date, status)
         VALUES ($1, $2, $3, $4, $5, $6, $7)`,
        [overdueId, uuidv4(), memberId, 500000, 500000, overdueDate.toISOString().split('T')[0], 'OPEN']
      );
    });
  ...
  describe('GET /api/piutang/alerts/overdue - Overdue Piutang Alerts (Requirement 18.8)', () => {
    ...
    it('should include days overdue in alert', async () => {
      ...
      if (res.body.data.length > 0) {
        const alert = res.body.data.find((a: any) => a.id === overdueId);
        if (alert) {
