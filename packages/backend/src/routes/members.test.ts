...
describe('Member Routes', () => {
  ...
  describe('GET /api/members', () => {
    it('should return list of members with pagination', async () => {
      const result = await db.query(
        'SELECT * FROM members WHERE is_active = true LIMIT 20 OFFSET 0'
      );
    ...
    it('should filter members by name', async () => {
      const result = await db.query(
        'SELECT * FROM members WHERE is_active = true AND name ILIKE $1',
        ['%Test%']
      );
      expect(result.rows.length).toBeGreaterThan(0);
      expect(result.rows.some((m: any) => m.id === testMemberId)).toBe(true);
    });

    it('should filter members by member number', async () => {
      const result = await db.query(
        'SELECT * FROM members WHERE is_active = true AND member_number ILIKE $1',
        ['%MBR_TEST%']
      );
      expect(result.rows.length).toBeGreaterThan(0);
      expect(result.rows.some((m: any) => m.id === testMemberId)).toBe(true);
    ...
    it('should filter members by phone', async () => {
      const result = await db.query(
        'SELECT * FROM members WHERE is_active = true AND phone ILIKE $1',
        ['%081234567890%']
      );
      expect(result.rows.length).toBeGreaterThan(0);
      expect(result.rows.some((m: any) => m.id === testMemberId)).toBe(true);
  ...
  describe('PUT /api/members/:id/credit', () => {
    it('should increase credit balance with TOPUP', async () => {
      ...
      const result = await db.query(
        ...
        [expectedBalance.toString(), new Date(), testMemberId]
      );
    ...
    it('should update member updated_at timestamp', async () => {
      ...
      const result = await db.query(
        'UPDATE members SET credit_balance = $1, updated_at = $2 WHERE id = $3 RETURNING updated_at',
        ['500000', new Date(), testMemberId]
      );
