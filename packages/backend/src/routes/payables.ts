...
router.get('/', authorize('OWNER'), async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Error fetching payables list:', error as Error, {
...
router.get('/:id', authorize('OWNER'), async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Error fetching payable:', error as Error, {
      requestId: req.requestId,
      userId: req.user?.id,
 ...
 */
router.put('/:id/payment', authorize('OWNER'), async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
...
router.get('/alerts/upcoming', authorize('OWNER'), async (req: Request, res: Response) => {
  try {
    ...
    const upcomingPayables = result.rows.map((row: any) => ({
      id: row.id,
      purchaseOrderId: row.purchase_order_id,
      supplierId: row.supplier_id,
      supplierName: row.supplier_name,
    ...
    logger.error('Error fetching upcoming payables:', error as Error, {
      requestId: req.requestId,
      userId: req.user?.id,
    });
...
router.get('/alerts/overdue', authorize('OWNER'), async (req: Request, res: Response) => {
  try {
    ...
    const overduePayables = result.rows.map((row: any) => ({
      id: row.id,
      purchaseOrderId: row.purchase_order_id,
      supplierId: row.supplier_id,
      supplierName: row.supplier_name,
    ...
    logger.error('Error fetching overdue payables:', error as Error, {
      requestId: req.requestId,
      userId: req.user?.id,
    });
