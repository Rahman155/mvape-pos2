...
import { processBatchSync, getSyncStats, type SyncRequestItem } from '../services/sync.js';
...
router.post('/batch', requireAuth(), async (req: Request, res: Response) => {
  try {
    ...
    for (let i = 0; i < items.length; i++) {
      const item = items[i] as SyncRequestItem;
      if (!item.id) {
    ...
    // Process batch
    const response = await processBatchSync(items as SyncRequestItem[]);
...
export const syncRouter = router as ReturnType<typeof Router>;
