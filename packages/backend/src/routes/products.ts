...
interface ProductSearchResponse extends PaginatedResult<ProductWithStock> {
  total: number;
  page: number;
  limit: number;
  pages: number;
 ...
 */
function validatePaginationParams(limit?: string, offset?: string) {
 ...
 */
function getStoreId(user: any, queryStoreId?: string): string {
  if (user.role === 'KASIR') {
    if (!user.storeId) {
      throw new ApiError(
        'Kasir user has no assigned store',
  ...
  throw new ApiError(
    'Owner must specify storeId parameter',
    ApiErrorCode.INVALID_INPUT,
    400
  );
}
...
function buildSearchQuery(
  searchTerm?: string,
  categoryFilter?: string
): { whereClause: string; values: (string | undefined)[] } {
...
productsRouter.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const storeId = getStoreId(req.user, req.query.storeId as string);
      ...
      const productsResult = await db.query<ProductWithStock>(
        searchQuery,
        queryValues
      );
...
productsRouter.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const result = await db.query<ProductWithStock>(query, [storeId, productId]);
...
productsRouter.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const result = await db.query<ProductWithStock>(query, [
        storeId,
        limit,
        offset,
      ]);
