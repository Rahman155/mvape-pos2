...
jest.mock('../middleware/authenticate.js', () => ({
  authenticateMiddleware: (req: any, res: any, next: any) => {
...
describe('BOP Expense Report Routes (Requirements 17.1-17.3)', () => {
  ...
  beforeEach(async () => {
    ...
    await db.query(
      ...
      [storeId1, 'Toko Jakarta', true]
    );

    await db.query(
      `INSERT INTO stores (id, name, is_active) VALUES ($1, $2, $3)
       ON CONFLICT(id) DO NOTHING`,
      [storeId2, 'Toko Bandung', true]
    );
    ...
    await db.query(
      `INSERT INTO bop (id, store_id, name, description, amount, effective_from, effective_to, created_at, updated_at)
       ...
       ON CONFLICT(id) DO NOTHING`,
      [bopId1, storeId1, 'Listrik', 'Biaya Listrik', 500000, '2024-01-01', null, new Date(), new Date()]
    );
    ...
    await db.query(
      `INSERT INTO bop (id, store_id, name, description, amount, effective_from, effective_to, created_at, updated_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
       ON CONFLICT(id) DO NOTHING`,
      [bopId2, storeId1, 'Air', 'Biaya Air', 300000, '2024-01-01', null, new Date(), new Date()]
    );
    ...
    await db.query(
      `INSERT INTO bop (id, store_id, name, description, amount, effective_from, effective_to, created_at, updated_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
       ON CONFLICT(id) DO NOTHING`,
      [bopId3, storeId2, 'Listrik', 'Biaya Listrik', 600000, '2024-01-01', null, new Date(), new Date()]
    );
    ...
    await db.query(
      `INSERT INTO bop (id, store_id, name, description, amount, effective_from, effective_to, created_at, updated_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
       ON CONFLICT(id) DO NOTHING`,
      [bopId4, storeId1, 'Internet', 'Biaya Internet', 200000, '2023-01-01', '2023-12-31', new Date(), new Date()]
    );
  });
  ...
  describe('Daily BOP Report', () => {
    ...
    it('should aggregate BOP by store for daily report', async () => {
      ...
      const storeData = response.body.data.byStore.find((s: any) => s.storeId === storeId1);
      expect(storeData).toBeDefined();
      expect(storeData.totalBOP).toBe(800000); // 500k + 300k
      expect(storeData.bopItems).toHaveLength(2);
      expect(storeData.bopItems).toContainEqual(
  ...
  describe('Monthly BOP Report', () => {
    ...
    it('should aggregate BOP by store for monthly report', async () => {
      ...
      const store1Data = response.body.data.byStore.find((s: any) => s.storeId === storeId1);
