...
router.get('/', authorize('OWNER'), async (req: Request, res: Response) => {
  try {
    ...
    const purchaseOrders = result.rows.map((row: any) => ({
      id: row.id,
    ...
    logger.error('Error fetching purchase orders list:', error as Error, {
...
router.post('/', authorize('OWNER'), async (req: Request, res: Response) => {
  try {
    ...
    const poItems = itemsResult.rows.map((row: any) => ({
      id: row.id,
    ...
    logger.error('Error creating purchase order:', error as Error, {
      requestId: req.requestId,
      userId: req.user?.id,
    });
...
router.get('/:id', authorize('OWNER'), async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Error fetching purchase order:', error as Error, {
      requestId: req.requestId,
      userId: req.user?.id,
      poId: req.params.id,
    });
