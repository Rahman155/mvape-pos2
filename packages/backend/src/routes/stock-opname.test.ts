...
let testOpnameSessionId: string;

// Helper function to generate test JWT token
const generateToken = (userId: string, role: string = 'OWNER') => {
  ...
  beforeAll(async () => {
    ...
    const userResult = await db.query(
      `INSERT INTO users (username, email, password_hash, role) 
       VALUES ($1, $2, $3, $4) 
       RETURNING id`,
      ['testuser', 'test@example.com', 'hashedpassword', 'OWNER']
    );
    ...
    const storeResult = await db.query(
      `INSERT INTO stores (name, address) 
       VALUES ($1, $2) 
       RETURNING id`,
      ['Test Store', 'Test Address']
    );
    testStoreId = storeResult.rows[0].id;

    const productResult = await db.query(
      `INSERT INTO products (name, sku, category, cost_price, selling_price) 
       VALUES ($1, $2, $3, $4, $5) 
       RETURNING id`,
      ['Test Product', 'TEST-001', 'Vape', 10000, 15000]
    );
  ...
  describe('POST /api/stock-opname/initiate', () => {
    ...
    test('Should create opname details for all products', async () => {
      ...
      const detailsResult = await db.query(
        'SELECT COUNT(*) as count FROM opname_details WHERE opname_id = $1',
        [opnameId]
      );
  ...
  describe('POST /api/stock-opname/:sessionId/complete', () => {
    ...
    test('Should record timestamp and verifier info', async () => {
      ...
      const result = await db.query(
        'SELECT verified_by, status FROM stock_opnames WHERE id = $1',
        [sessionId]
      );
