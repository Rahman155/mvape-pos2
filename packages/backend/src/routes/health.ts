...
export const healthRouter = Router() as ReturnType<typeof Router>;
 ...
 */
healthRouter.get('/cache/status', async (_req: Request, res: Response) => {
  try {
    ...
    logger.error('Cache status check failed', error as Error);
    res.status(500).json({
