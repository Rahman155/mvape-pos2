...
export class AuthService {
    ...
    static async login(loginRequest) {
        try {
            ...
            // Find user by email
            const result = await db.query('SELECT id, username, email, password_hash, role, store_id, is_active FROM users WHERE email = $1', [email]);
            ...
            if (user.role === 'KASIR') {
                try {
                    ...
                    logger.warn('Failed to record attendance during login', attendanceError, {
                        userId: user.id,
                    });
    ...
    static async refreshAccessToken(refreshToken) {
        try {
            ...
            const result = await db.query('SELECT id, username, email, role, store_id, is_active FROM users WHERE id = $1', [payload.userId]);
            if (!result.rows || result.rows.length === 0) {
     ...
     */
    static getTokenExpiresIn(expiryString) {
    ...
    static async validateUser(userId) {
        try {
            const result = await db.query('SELECT id, username, email, role, store_id, is_active FROM users WHERE id = $1', [userId]);
            if (!result.rows || result.rows.length === 0) {
                throw new ApiError('User not found', ApiErrorCode.UNAUTHORIZED, 401);
            }
            const user = result.rows[0];
