...
export class AttendanceService {
     ...
     */
    static async clockIn(request) {
        try {
            const { userId, clockInTime = new Date() } = request;
            // Validate user exists and has KASIR role
            const userResult = await db.query('SELECT id, role, is_active FROM users WHERE id = $1', [userId]);
            ...
            const incompleteResult = await db.query(`SELECT id, clock_in, date FROM attendance 
         WHERE user_id = $1 AND clock_out IS NULL AND status = 'PRESENT' AND date < $2
         ORDER BY date DESC LIMIT 1`, [userId, date]);
            if (incompleteResult.rows && incompleteResult.rows.length > 0) {
                const incompleteRecord = incompleteResult.rows[0];
                const previousClockIn = new Date(incompleteRecord.clock_in);
                const durationMinutes = Math.round((clockInTime.getTime() - previousClockIn.getTime()) / 60000);
                // Update the incomplete record: set clock_out and duration
                await db.query(`UPDATE attendance 
           SET clock_out = $1, duration_minutes = $2, status = 'PRESENT', updated_at = NOW()
           WHERE id = $3`, [clockInTimeISO, durationMinutes, incompleteRecord.id]);
            ...
            const existingResult = await db.query('SELECT id, clock_in FROM attendance WHERE user_id = $1 AND date = $2', [userId, date]);
            ...
            const insertResult = await db.query(`INSERT INTO attendance (id, user_id, clock_in, date, status, created_at)
         VALUES ($1, $2, $3, $4, $5, NOW())
         RETURNING id, user_id, clock_in, date, status, created_at`, [attendanceId, userId, clockInTimeISO, date, 'PRESENT']);
            if (!insertResult.rows || insertResult.rows.length === 0) {
                throw new ApiError('Failed to create attendance record', ApiErrorCode.DATABASE_ERROR, 500);
            }
     ...
     */
    static async clockOut(userId, clockOutTime = new Date()) {
        try {
            const date = this.formatDate(clockOutTime);
            const clockOutTimeISO = clockOutTime.toISOString();
            // Find today's attendance record
            const selectResult = await db.query('SELECT id, clock_in, date FROM attendance WHERE user_id = $1 AND date = $2', [userId, date]);
            if (!selectResult.rows || selectResult.rows.length === 0) {
                logger.warn('No attendance record found for clock-out', {
                    userId,
                    date,
                });
                throw new ApiError('No attendance record found for today', ApiErrorCode.NOT_FOUND, 404);
            }
            const record = selectResult.rows[0];
            // Calculate duration in minutes
            const clockInTime = new Date(record.clock_in);
            const clockOutTimeDate = new Date(clockOutTimeISO);
            const durationMinutes = Math.round((clockOutTimeDate.getTime() - clockInTime.getTime()) / 60000);
            // Update attendance record with clock-out time
            const updateResult = await db.query(`UPDATE attendance 
         SET clock_out = $1, duration_minutes = $2, status = $3, updated_at = NOW()
         WHERE id = $4
         RETURNING id, user_id, clock_in, clock_out, duration_minutes, date, status, created_at`, [clockOutTimeISO, durationMinutes, 'PRESENT', record.id]);
            if (!updateResult.rows || updateResult.rows.length === 0) {
                throw new ApiError('Failed to update attendance record', ApiErrorCode.DATABASE_ERROR, 500);
            }
     ...
     */
    static async getTodaysAttendance(userId, date = new Date()) {
        try {
            const dateStr = this.formatDate(date);
            const result = await db.query('SELECT id, user_id, clock_in, clock_out, duration_minutes, date, status, created_at FROM attendance WHERE user_id = $1 AND date = $2', [userId, dateStr]);
            ...
            throw new ApiError('Failed to retrieve attendance record', ApiErrorCode.INTERNAL_ERROR, 500);
        }
    }
    /**
     ...
     */
    static async getAttendanceByPeriod(userId, startDate, endDate, limit = 100, offset = 0) {
        try {
            const startDateStr = this.formatDate(startDate);
            const endDateStr = this.formatDate(endDate);
            let countQuery = 'SELECT COUNT(*) as count FROM attendance WHERE date >= $1 AND date <= $2';
            let countParams = [startDateStr, endDateStr];
            let dataQuery = `SELECT id, user_id, clock_in, clock_out, duration_minutes, date, status, created_at 
                      ...
                      LIMIT $3 OFFSET $4`;
            let dataParams = [startDateStr, endDateStr, limit, offset];
            ...
            throw new ApiError('Failed to retrieve attendance records', ApiErrorCode.INTERNAL_ERROR, 500);
        }
    }
    /**
     ...
     */
    static async getAttendanceWithUserInfo(userId, startDate, endDate, limit = 100, offset = 0) {
        try {
            const startDateStr = this.formatDate(startDate);
            const endDateStr = this.formatDate(endDate);
            let countQuery = `
      ...
      `;
        ...
        `;
            ...
            logger.error('Failed to get attendance records with user info', error);
            throw new ApiError('Failed to retrieve attendance records', ApiErrorCode.INTERNAL_ERROR, 500);
        }
    }
    /**
     ...
     */
    static async getAttendanceSummary(startDate, endDate, storeId = null) {
        try {
            const startDateStr = this.formatDate(startDate);
            const endDateStr = this.formatDate(endDate);
            let query = `
      ...
      `;
            ...
            throw new ApiError('Failed to retrieve attendance summary', ApiErrorCode.INTERNAL_ERROR, 500);
        }
    }
    /**
    ...
    static async getMonthlyReport(year, month) {
      ...
      `;
            ...
            if (!result.rows) {
                throw new ApiError('Failed to generate report', ApiErrorCode.DATABASE_ERROR, 500);
            }
            // Process results
            const report = result.rows.map((row) => {
                ...
                const averageHoursPerDay = totalWorkDays > 0 ? parseFloat((totalHours / totalWorkDays).toFixed(2)) : 0;
                return {
                    userId: row.user_id,
            ...
            const totalHoursWorked = parseFloat(report.reduce((sum, r) => sum + r.totalHours, 0).toFixed(2));
            ...
            throw new ApiError('Failed to generate report', ApiErrorCode.INTERNAL_ERROR, 500);
        }
    }
}
