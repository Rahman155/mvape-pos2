 ...
 */
function determineStrategy(entityType, clientData, serverData, changeType) {
 ...
 */
function canMergeMemberChanges(clientData, serverData) {
 ...
 */
function canMergeBopChanges(clientData, serverData) {
    if (!clientData || !serverData) {
        return false;
    }
 ...
 */
export function resolveLWW(conflict) {
    ...
    logger.info(`[LWW] Conflict for ${entityType} ${id}: client(${normalizedClientTs}) vs server(${normalizedServerTs}) → ${winner} wins`);
    return {
        conflictId: id,
        entityType,
        strategy: 'LWW',
 ...
 */
function resolveMergeMember(clientData, serverData) {
 ...
 * IDEMPOTENT: Same inputs always produce same merged result
 */
function resolveMergeBop(clientData, serverData) {
 ...
 */
export function resolveManual(conflict) {
    const { id, entityType, clientTimestamp, serverTimestamp } = conflict;
    logger.warn(`[MANUAL] Conflict requires user review for ${entityType} ${id}: ` +
 ...
 */
function updateStats(resolution) {
    conflictStats.total++;
...
export function resolveConflict(conflict) {
    ...
    const strategy = determineStrategy(normalizedConflict.entityType, normalizedConflict.clientData, normalizedConflict.serverData);
    logger.info(`Resolving conflict ${normalizedConflict.id} (${normalizedConflict.entityType}) using ${strategy} strategy`);
    let resolution;
    switch (strategy) {
        case 'LWW':
 ...
 */
export function resolveBatchConflicts(conflicts) {
 ...
 */
export function hasConflict(clientData, serverData) {
    if (!clientData || !serverData) {
        return false;
    }
...
export function applyResolution(resolution) {
    ...
    logger.info(`Applying ${resolution.strategy} resolution for ${resolution.entityType} ` +
 ...
 */
export function verifyIdempotency(conflict) {
    try {
        ...
        if (!identical) {
            logger.warn(`Idempotency check FAILED for ${conflict.id}: ` +
