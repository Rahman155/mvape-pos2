 ...
 */
export async function processBatchSync(items) {
    const results = [];
    ...
    logger.info(`Batch sync completed: ${successCount} success, ${failureCount} failed, ${conflictsDetected} conflicts, ${processingTime}ms`);
    return {
 ...
 */
async function detectConflict(id, entityType, clientTimestamp, changeType) {
    ...
    try {
        ...
        if (result.rows && result.rows.length > 0) {
            ...
            if (hasConflict) {
                logger.info(`Conflict detected for ${entityType} ${id}: server=${serverTimestamp}, client=${clientTimestamp}`);
                return {
                    hasConflict: true,
                    strategy: 'LWW',
                    serverTimestamp,
    ...
    try {
        ...
        if (changeType !== 'CREATE') {
            ...
            if (conflictInfo.hasConflict) {
                logger.info(`Conflict detected and will be resolved using ${conflictInfo.strategy} strategy for ${entityType} ${id}`);
                ...
                notificationManager.notifyConflict(entityType, id, strategy, conflictResolution.reason, conflictResolution.requiresUserReview);
 ...
 */
async function processSyncTransaction(id, changeType, data) {
    if (changeType !== 'CREATE') {
    ...
    const transactionResult = await db.transaction(async (client) => {
        ...
        const { storeId, kasirId, items, paymentMethod, paymentData, notes, } = data;
        ...
        if (paymentMethod === 'CASH') {
            ...
            const creditValidation = await validateMemberCredit(paymentData.memberCredit.memberId, data.totalAmount || 0);
        ...
        // Insert transaction
        const insertTxResult = await client.query(`INSERT INTO transactions (
        id, store_id, kasir_id, transaction_date, total_amount,
        payment_method, status
      ) VALUES ($1, $2, $3, $4, $5, $6, $7)
      RETURNING *`, [transactionId, storeId, kasirId, new Date(), totalAmount, paymentMethod, 'COMPLETED']);
        ...
        WHERE product_id = $2 AND store_id = $3`, [item.quantity, item.productId, storeId]);
        }
        ...
        if (paymentMethod === 'MEMBER_CREDIT') {
            ...
            await client.query(`UPDATE members SET credit_balance = credit_balance - $1
        WHERE id = $2`, [usedCredit, memberId]);
        }
        else if (paymentMethod === 'TEMPO') {
            // Create piutang record
            const piutangId = uuidv4();
            await client.query(`INSERT INTO piutang (
 ...
 */
async function processSyncMember(id, changeType, data) {
    if (changeType !== 'CREATE') {
    ...
    const result = await db.query(`INSERT INTO members (id, member_number, name, phone, email, credit_balance)
    VALUES ($1, $2, $3, $4, $5, 0)
    RETURNING *`, [memberId, memberNumber, name, phone, email || null]);
 ...
 */
async function processSyncProduct(id, changeType, data) {
