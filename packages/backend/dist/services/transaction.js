 ...
 */
export async function validateMemberCredit(memberId, requiredAmount) {
    try {
        const result = await db.query('SELECT credit_balance FROM members WHERE id = $1 AND is_active = true', [memberId]);
 ...
 */
export async function deductMemberCredit(memberId, amount) {
    try {
        // Start transaction
        await db.query('BEGIN');
        // Get current balance
        const memberResult = await db.query('SELECT credit_balance FROM members WHERE id = $1 FOR UPDATE', [memberId]);
        ...
        const updateResult = await db.query('UPDATE members SET credit_balance = $1, updated_at = $2 WHERE id = $3 RETURNING credit_balance', [newBalance.toString(), now, memberId]);
...
export async function createTransaction(request) {
    try {
        ...
        if (request.paymentMethod === 'MEMBER_CREDIT') {
            ...
            const validation = await validateMemberCredit(request.paymentData.memberCredit.memberId, request.paymentData.memberCredit.usedCredit);
            if (!validation.valid) {
                return {
                    success: false,
        ...
        try {
            ...
            const txnResult = await db.query(`INSERT INTO transactions 
         ...
         RETURNING *`, [
           ...
           RETURNING *`, [
                ...
                // Deduct inventory
                await db.query('UPDATE inventory SET quantity = quantity - $1 WHERE product_id = $2 AND store_id = $3', [item.quantity, item.productId, request.storeId]);
            }
            // Handle payment-specific logic
            if (request.paymentMethod === 'MEMBER_CREDIT') {
                const creditData = request.paymentData.memberCredit;
                const deductResult = await deductMemberCredit(creditData.memberId, creditData.usedCredit);
