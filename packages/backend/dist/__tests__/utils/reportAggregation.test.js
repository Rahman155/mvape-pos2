 ...
 */
import { calculateDailyStatistics, calculateMultiStoreStatistics, calculateAggregatedStatistics, getDayBoundaries, } from '../../utils/dashboard.js';
...
describe('Report Aggregation Tests', () => {
    ...
    describe('Daily Aggregation Correctness (Requirement 23)', () => {
        ...
        it('should only count COMPLETED transactions in daily aggregation', async () => {
            ...
            expect(mockDb.query).toHaveBeenCalledWith(expect.stringContaining('status = $4'), expect.arrayContaining([storeId, date, date, 'COMPLETED']));
        });
    ...
    describe('Weekly Aggregation with Various Dates (Requirement 24)', () => {
        it('should aggregate full week (Monday to Sunday)', async () => {
            ...
            const result = await calculateDailyStatistics(storeId, weekStart, weekEnd);
        ...
        it('should provide daily breakdown within week', async () => {
            ...
            for (let i = 0; i < 7; i++) {
                ...
                const stats = await calculateDailyStatistics(storeId, dayStart, dayEnd);
                days.push(stats);
            }
        ...
        it('should aggregate data per store for weekly period', async () => {
            ...
            const results = await calculateMultiStoreStatistics(storeIds, weekStart, weekEnd);
        ...
        it('should show weekly trends for multiple weeks', async () => {
            ...
            const week1 = await calculateDailyStatistics(storeId, new Date('2024-01-01'), new Date('2024-01-07'));
            const week2 = await calculateDailyStatistics(storeId, new Date('2024-01-08'), new Date('2024-01-14'));
            const week3 = await calculateDailyStatistics(storeId, new Date('2024-01-15'), new Date('2024-01-21'));
    ...
    describe('Monthly Aggregation Edge Cases (Requirement 25)', () => {
        ...
        it('should handle different month lengths (28, 30, 31 days)', async () => {
            ...
            const feb = await calculateDailyStatistics(storeId, new Date('2024-02-01'), new Date('2024-02-28'));
            const apr = await calculateDailyStatistics(storeId, new Date('2024-04-01'), new Date('2024-04-30'));
            const may = await calculateDailyStatistics(storeId, new Date('2024-05-01'), new Date('2024-05-31'));
        ...
        it('should provide weekly breakdown within month', async () => {
            ...
            for (let week = 1; week <= 4; week++) {
                ...
                const stats = await calculateDailyStatistics(storeId, startDate, endDate);
                weeks.push(stats);
            }
        ...
        it('should aggregate data per store for monthly period', async () => {
            ...
            const results = await calculateMultiStoreStatistics(storeIds, startDate, endDate);
            expect(results.size).toBe(3);
        ...
        it('should provide monthly breakdown across multiple months', async () => {
            ...
            const jan = await calculateDailyStatistics(storeId, new Date('2024-01-01'), new Date('2024-01-31'));
            const feb = await calculateDailyStatistics(storeId, new Date('2024-02-01'), new Date('2024-02-28'));
            const mar = await calculateDailyStatistics(storeId, new Date('2024-03-01'), new Date('2024-03-31'));
    ...
    describe('Multiple Store Report Aggregation', () => {
        ...
        it('should handle aggregation for 10+ stores', async () => {
            const storeIds = Array.from({ length: 15 }, (_, i) => `store-${i + 1}`);
            const date = new Date('2024-01-15');
    ...
    describe('Report Aggregation Consistency', () => {
        it('should maintain consistency between daily and weekly totals', async () => {
            ...
            const day1 = await calculateDailyStatistics(storeId, new Date('2024-01-15'), new Date('2024-01-15'));
            const day2 = await calculateDailyStatistics(storeId, new Date('2024-01-16'), new Date('2024-01-16'));
            const day3 = await calculateDailyStatistics(storeId, new Date('2024-01-17'), new Date('2024-01-17'));
            const weekTotal = await calculateDailyStatistics(storeId, new Date('2024-01-15'), new Date('2024-01-21'));
        ...
        it('should maintain consistency between weekly and monthly totals', async () => {
            ...
            const week1 = await calculateDailyStatistics(storeId, new Date('2024-01-01'), new Date('2024-01-07'));
            const week2 = await calculateDailyStatistics(storeId, new Date('2024-01-08'), new Date('2024-01-14'));
            const monthTotal = await calculateDailyStatistics(storeId, new Date('2024-01-01'), new Date('2024-01-31'));
        ...
        it('should ensure average transaction value is consistent', async () => {
            ...
            const result = await calculateDailyStatistics(storeId, date, date);
            expect(result.averageTransactionValue).toBe(result.totalSales / result.transactionCount);
        });
    });
});
//# sourceMappingURL=reportAggregation.test.js.map