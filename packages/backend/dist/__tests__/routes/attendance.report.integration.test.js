...
describe('GET /api/v1/attendance/report - Monthly Attendance Report', () => {
    ...
    beforeAll(async () => {
        ...
        // Create test store
        storeId = uuidv4();
        await db.query('INSERT INTO stores (id, name, address) VALUES ($1, $2, $3) ON CONFLICT DO NOTHING', [storeId, 'Test Store', 'Test Address']);
        // Create owner user
        ownerUserId = uuidv4();
       ...
       ON CONFLICT DO NOTHING`, [
        ...
        const ownerAuthResult = await AuthService.login((await db.query('SELECT email FROM users WHERE id = $1', [ownerUserId])).rows[0].email, ownerPassword);
        ownerToken = 'Bearer ' + ownerAuthResult.accessToken;
        const kasirAuthResult = await AuthService.login((await db.query('SELECT email FROM users WHERE id = $1', [kasirUserId1])).rows[0].email, kasirPassword);
    ...
    describe('Employee Report Data', () => {
        ...
        it('should calculate correct total work days', async () => {
            const res = await request(app)
                ...
                .set('Authorization', ownerToken);
            if (res.status === 200) {
                const kasir1 = res.body.data.report.find((r) => r.userId === kasirUserId1);
                const kasir2 = res.body.data.report.find((r) => r.userId === kasirUserId2);
    ...
    describe('Edge Cases', () => {
        ...
        it('should handle decimal hours correctly', async () => {
            ...
            await db.query(`INSERT INTO attendance (id, user_id, clock_in, clock_out, duration_minutes, date, status, created_at)
         VALUES ($1, $2, $3, $4, $5, $6, $7, NOW())`, [uuidv4(), kasirUserId1, clockIn, clockOut, 450, '2024-02-01', 'PRESENT']);
            const res = await request(app)
                .get('/api/v1/attendance/report?year=2024&month=2')
                .set('Authorization', ownerToken);
            if (res.status === 200) {
        ...
        it('should only include KASIR users in report', async () => {
            ...
            if (res.status === 200) {
                ...
                const ownerInReport = report.find((r) => r.userId === ownerUserId);
        ...
        it('should only include records with clock_out time', async () => {
            ...
            await db.query(`INSERT INTO attendance (id, user_id, clock_in, clock_out, duration_minutes, date, status, created_at)
         VALUES ($1, $2, $3, $4, $5, $6, $7, NOW())`, [uuidv4(), kasirUserId1, clockIn, null, null, '2024-02-15', 'INCOMPLETE']);
            const res = await request(app)
                .get('/api/v1/attendance/report?year=2024&month=2')
                .set('Authorization', ownerToken);
            if (res.status === 200) {
...
//# sourceMappingURL=attendance.report.integration.test.js.map