...
describe('Member Credit Top-up Endpoint', () => {
    ...
    beforeAll(async () => {
       ...
       ON CONFLICT DO NOTHING`, [testOwnerId, 'testowner', `owner_${Date.now()}@test.com`, hashedPassword, 'OWNER', testStoreId, true]);
        // Create KASIR user
        await db.query(`INSERT INTO users (id, username, email, password_hash, role, store_id, is_active) 
       VALUES ($1, $2, $3, $4, $5, $6, $7) 
       ...
       ON CONFLICT DO NOTHING`, [testMemberId, 'MBR001', 'Test Member', '08123456789', 'member@test.com', '100', '0', true, new Date(), new Date()]);
        // Get auth tokens
    ...
    describe('POST /api/members/:id/topup', () => {
        ...
        it('should record transaction in credit_transactions table', async () => {
            ...
            const transactionResult = await db.query('SELECT * FROM credit_transactions WHERE member_id = $1 ORDER BY created_at DESC LIMIT 1', [testMemberId]);
        ...
        it('should handle multiple top-ups correctly', async () => {
            ...
            const transactionResult = await db.query('SELECT COUNT(*) as count FROM credit_transactions WHERE member_id = $1 AND transaction_type = $2', [testMemberId, 'TOPUP']);
            expect(Number(transactionResult.rows[0].count)).toBeGreaterThanOrEqual(2);
        });
    });
});
//# sourceMappingURL=members.topup.test.js.map