...
describe('Piutang Management (Tasks 73-77)', () => {
    ...
    beforeAll(async () => {
       ...
       ON CONFLICT DO NOTHING`, [kasirUserId, 'kasir_user', `kasir_${Date.now()}@test.com`, hashedPassword, 'KASIR', testStoreId, true]);
        ...
        // Create customer
        await db.query(`INSERT INTO members (id, member_number, name, phone, email, is_active, created_at, updated_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
       ON CONFLICT DO NOTHING`, [customerId, 'CUST001', 'John Doe', '08123456789', 'john@example.com', true, new Date(), new Date()]);
    ...
    describe('Task 73: GET /api/piutang - List with Filtering', () => {
        beforeEach(async () => {
            ...
            // Create customers
            await db.query(`INSERT INTO members (id, member_number, name, phone, email, is_active, created_at, updated_at)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`, [testCustomer1, 'CUST002', 'Alice Smith', '08111111111', 'alice@example.com', true, new Date(), new Date()]);
            await db.query(`INSERT INTO members (id, member_number, name, phone, email, is_active, created_at, updated_at)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`, [testCustomer2, 'CUST003', 'Bob Johnson', '08222222222', 'bob@example.com', true, new Date(), new Date()]);
        ...
        it('should support filtering by due date range', async () => {
            ...
            const response = await request(app)
                .get(`/api/piutang?dueDateFrom=${fromDate.toISOString().split('T')[0]}&dueDateTo=${toDate.toISOString().split('T')[0]}`)
                .set('Authorization', `Bearer ${userToken}`);
            expect(response.status).toBe(200);
            response.body.data.forEach((item) => {
        ...
        it('should support sorting by due_date (default)', async () => {
            ...
            for (let i = 1; i < response.body.data.length; i++) {
                if (response.body.data[i - 1].dueDate && response.body.data[i].dueDate) {
                    expect(new Date(response.body.data[i - 1].dueDate).getTime()).toBeLessThanOrEqual(new Date(response.body.data[i].dueDate).getTime());
                }
            }
        });
        it('should support sorting by remaining_balance', async () => {
            ...
            for (let i = 1; i < response.body.data.length; i++) {
                expect(response.body.data[i - 1].remainingBalance).toBeGreaterThanOrEqual(response.body.data[i].remainingBalance);
            }
        });
        it('should support sorting by created_date', async () => {
            ...
            // Verify sorted (descending)
            for (let i = 1; i < response.body.data.length; i++) {
                expect(new Date(response.body.data[i - 1].createdAt).getTime()).toBeGreaterThanOrEqual(new Date(response.body.data[i].createdAt).getTime());
            }
        });
    ...
    describe('Task 75: POST /api/piutang/:id/payment - Payment Recording', () => {
        ...
        it('should accept any valid payment amount between 0 (exclusive) and remaining_balance (inclusive)', async () => {
            await fc.assert(fc.asyncProperty(fc.integer({ min: 1, max: 500000 }), async (paymentAmount) => {
    ...
    describe('Task 76: Status Management (OPEN/PARTIAL/CLOSED Workflow)', () => {
        ...
        it('should maintain correct status after any valid payment sequence', async () => {
            await fc.assert(fc.asyncProperty(fc.array(fc.integer({ min: 1, max: 100000 }), { minLength: 1 }).map((arr) => {
    ...
    describe('Edge Cases & Boundary Conditions', () => {
        ...
        it('should handle piutang with NULL due_date in list', async () => {
            ...
            await db.query(`INSERT INTO members (id, member_number, name, phone, email, is_active, created_at, updated_at)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`, [testCustomer, 'CUST_NULLDATE', 'Null Date Customer', '081234567890', 'null@example.com', true, new Date(), new Date()]);
            await db.query(`INSERT INTO piutang (id, transaction_id, member_id, amount, remaining_balance, due_date, status, created_at, updated_at)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`, [
                uuidv4(),
                uuidv4(),
