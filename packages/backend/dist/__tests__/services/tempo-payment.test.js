...
describe('Tempo Payment Processing', () => {
    ...
    beforeEach(async () => {
        ...
        try {
            ...
            // Create kasir user
            await db.query('INSERT INTO users (id, username, email, password_hash, role, store_id) VALUES ($1, $2, $3, $4, $5, $6)', [testKasirId, 'test_kasir', 'test@example.com', 'hashed_password', 'KASIR', testStoreId]);
            // Create product
            await db.query('INSERT INTO products (id, name, sku, cost_price, selling_price) VALUES ($1, $2, $3, $4, $5)', [testProductId, 'Test Product', 'SKU001', '10000', '15000']);
            // Create inventory
    ...
    describe('Piutang Record Creation', () => {
        it('should create piutang record when tempo transaction is confirmed', async () => {
            ...
            if (result.success && result.transaction) {
                ...
                const piutangResult = await db.query('SELECT * FROM piutang WHERE transaction_id = $1', [result.transaction.id]);
        ...
        it('should set piutang status to OPEN for new tempo transaction', async () => {
            ...
            const result = await createTransaction(request);
            if (result.success && result.transaction) {
                const piutangResult = await db.query('SELECT status FROM piutang WHERE transaction_id = $1', [result.transaction.id]);
        ...
        it('should set correct remaining_balance equal to amount for new piutang', async () => {
            ...
            const result = await createTransaction(request);
            if (result.success && result.transaction) {
                const piutangResult = await db.query('SELECT amount, remaining_balance FROM piutang WHERE transaction_id = $1', [result.transaction.id]);
                const piutang = piutangResult.rows[0];
    ...
    describe('Inventory Deduction on Tempo Transaction', () => {
        it('should deduct inventory when tempo transaction is created', async () => {
            ...
            const beforeResult = await db.query('SELECT quantity FROM inventory WHERE product_id = $1 AND store_id = $2', [testProductId, testStoreId]);
            ...
            const afterResult = await db.query('SELECT quantity FROM inventory WHERE product_id = $1 AND store_id = $2', [testProductId, testStoreId]);
    ...
    describe('Integration: Tempo Payment End-to-End', () => {
        it('should complete full tempo payment flow', async () => {
            ...
            if (result.transaction) {
                ...
                const piutangResult = await db.query('SELECT * FROM piutang WHERE transaction_id = $1', [transactionId]);
                expect(piutangResult.rows.length).toBe(1);
                const piutang = piutangResult.rows[0];
                expect(Number(piutang.amount)).toBe(totalAmount);
                expect(piutang.due_date).toBe(dueDate);
...
//# sourceMappingURL=tempo-payment.test.js.map