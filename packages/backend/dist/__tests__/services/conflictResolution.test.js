...
import { resolveConflict, resolveLWW, } from '../../services/conflictResolution.js';
describe('Conflict Resolution - Property 2: Idempotency', () => {
    describe('LWW Strategy Idempotency', () => {
        ...
        it('should resolve conflicts idempotently with LWW strategy (Property 2)', () => {
            const prop = fc.property(fc.object({ maxDepth: 2 }), fc.object({ maxDepth: 2 }), fc.integer({ min: 1000000000000, max: Date.now() }), fc.integer({ min: 1000000000000, max: Date.now() }), (clientData, serverData, clientTs, serverTs) => {
                ...
                expect(JSON.stringify(resolution1.resolvedData)).toBe(JSON.stringify(resolution2.resolvedData));
                expect(JSON.stringify(resolution2.resolvedData)).toBe(JSON.stringify(resolution3.resolvedData));
    ...
    describe('Generic Resolver Idempotency', () => {
        ...
        it('should resolve conflicts idempotently using generic resolver', () => {
            const prop = fc.property(fc.object({ maxDepth: 2 }), fc.object({ maxDepth: 2 }), fc.integer({ min: 1000000000000, max: Date.now() }), fc.integer({ min: 1000000000000, max: Date.now() }), fc.constantFrom('transaction', 'member', 'product'), (clientData, serverData, clientTs, serverTs, entityType) => {
                if (clientTs === serverTs) {
                    return true;
                }
                const conflict = {
    ...
    describe('Determinism Property', () => {
        ...
        it('should be deterministic for identical inputs', () => {
            const prop = fc.property(fc.object({ maxDepth: 2 }), fc.object({ maxDepth: 2 }), (clientData, serverData) => {
                const conflict = {
                    id: 'same-id',
                    entityType: 'transaction',
    ...
    describe('Winner Consistency Property', () => {
        ...
        it('should consistently select newer client version as winner', () => {
            const prop = fc.property(fc.object({ maxDepth: 2 }), fc.object({ maxDepth: 2 }), fc.integer({ min: 1000000000000, max: Date.now() - 1000 }), (clientData, serverData, baseTimestamp) => {
                ...
                expect(JSON.stringify(resolution.resolvedData)).toBe(JSON.stringify(clientData));
                return true;
            });
            fc.assert(prop, { numRuns: 100 });
        });
        it('should consistently select newer server version as winner', () => {
            const prop = fc.property(fc.object({ maxDepth: 2 }), fc.object({ maxDepth: 2 }), fc.integer({ min: 1000000000000, max: Date.now() - 1000 }), (clientData, serverData, baseTimestamp) => {
                ...
                expect(JSON.stringify(resolution.resolvedData)).toBe(JSON.stringify(serverData));
                return true;
            });
            fc.assert(prop, { numRuns: 100 });
        });
    ...
    describe('Data Integrity Property', () => {
        ...
        it('should never produce corrupted data', () => {
            const prop = fc.property(fc.object({ maxDepth: 2 }), fc.object({ maxDepth: 2 }), (clientData, serverData) => {
                ...
                const isClientData = JSON.stringify(resolution.resolvedData) === JSON.stringify(clientData);
                const isServerData = JSON.stringify(resolution.resolvedData) === JSON.stringify(serverData);
