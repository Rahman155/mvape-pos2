...
import { resolveConflict, } from '../../services/conflictResolution.js';
describe('Conflict Resolution Integration', () => {
    ...
    describe('Conflict Detection and Resolution Flow', () => {
        it('should detect conflict and apply LWW resolution', () => {
            ...
            expect(JSON.stringify(resolution.resolvedData)).toBe(JSON.stringify(conflictData.serverData));
        });
        it('should notify user when conflict is detected', () => {
            ...
            // Resolve and notify
            const resolution = resolveConflict(conflictData);
            notificationManager.notifyConflict(conflictData.entityType, conflictData.id, resolution.strategy, resolution.reason, resolution.requiresUserReview);
    ...
    describe('Batch Conflict Handling', () => {
        it('should handle multiple conflicts in sequence', () => {
            ...
            conflicts.forEach((conflict) => {
                ...
                notificationManager.notifyConflict(conflict.entityType, conflict.id, resolution.strategy, resolution.reason, resolution.requiresUserReview);
            });
    ...
    describe('User Notification Lifecycle', () => {
        it('should track conflict from creation to resolution', () => {
            ...
            const notification = notificationManager.notifyConflict('transaction', 'txn-789', 'LWW', 'Server version is newer');
            // Verify unresolved
        ...
        it('should distinguish between auto-resolved and manual conflicts', () => {
            ...
            notificationManager.notifyConflict('transaction', 'txn-1', 'LWW', 'Auto-resolved using LWW', false // No manual review needed
            );
            // Manual review conflict
            notificationManager.notifyConflict('transaction', 'txn-2', 'MANUAL', 'Requires manual review', true // Requires manual review
            );
            // Verify statistics
    ...
    describe('Conflict Audit Trail', () => {
        it('should maintain resolution history', () => {
            ...
            for (let i = 0; i < 3; i++) {
                const notification = notificationManager.notifyConflict('transaction', `txn-${i}`, 'LWW', `Conflict ${i}`);
    ...
    describe('Error Handling', () => {
        it('should handle merge resolution safely', () => {
            ...
            try {
                ...
                expect(resolution.resolvedData.total_spent).toBeGreaterThanOrEqual(Math.max(conflict.clientData.total_spent, conflict.serverData.total_spent));
            }
            catch (e) {
...
//# sourceMappingURL=conflictResolutionIntegration.test.js.map