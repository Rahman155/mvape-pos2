    ...
    try {
        ...
        await db.transaction(async (client) => {
            ...
            for (const store of seedData.stores) {
                await client.query(`INSERT INTO stores (id, name, address, phone, operating_hours, is_active)
           VALUES ($1, $2, $3, $4, $5, $6)`, [store.id, store.name, store.address, store.phone, JSON.stringify(store.operating_hours), store.is_active]);
            }
            // 2. Insert users
            ...
            for (let i = 0; i < seedData.users.length; i++) {
                ...
                await client.query(`INSERT INTO users (id, username, email, password_hash, role, store_id, is_active)
           VALUES ($1, $2, $3, $4, $5, $6, $7)`, [user.id, user.username, user.email, hashedPassword, user.role, storeId, user.is_active]);
            }
            // 3. Insert products
            logger.info('Seeding products...');
            for (const product of seedData.products) {
                await client.query(`INSERT INTO products (id, name, sku, category, cost_price, selling_price, description, is_active)
           VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`, [
            ...
            for (const store of seedData.stores) {
                for (const product of seedData.products) {
                    await client.query(`INSERT INTO inventory (id, product_id, store_id, quantity, reserved, reorder_level)
             VALUES ($1, $2, $3, $4, $5, $6)`, [uuidv4(), product.id, store.id, Math.floor(Math.random() * 50) + 10, 0, 5]);
                }
            }
            // 5. Insert members
            logger.info('Seeding members...');
            for (const member of seedData.members) {
                await client.query(`INSERT INTO members (id, member_number, name, phone, email, credit_balance, total_spent, is_active)
           VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`, [
                    member.id,
            ...
            for (const store of seedData.stores) {
                ...
                for (const bop of bopItems) {
                    await client.query(`INSERT INTO bop (id, store_id, name, amount, effective_from)
             VALUES ($1, $2, $3, $4, $5)`, [uuidv4(), store.id, bop.name, bop.amount, new Date().toISOString().split('T')[0]]);
                }
            }
            // 7. Insert suppliers
            ...
            for (const supplier of suppliers) {
                await client.query(`INSERT INTO suppliers (id, name, phone, email, address, is_active)
           VALUES ($1, $2, $3, $4, $5, $6)`, [supplier.id, supplier.name, supplier.phone, supplier.email, supplier.address, supplier.is_active]);
            }
        });
