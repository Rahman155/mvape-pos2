...
describe('Daily Sales Report Routes (Requirements 23.1-23.3)', () => {
    ...
    beforeEach(async () => {
        ...
        for (const store of stores) {
         ...
         ON CONFLICT(id) DO NOTHING`, [store.id, store.name, store.is_active]);
        }
        // Create test transactions for testDate
        const txnDate = new Date(testDate + 'T12:00:00Z');
        // Store 1: CASH transactions
        const tx1 = uuidv4();
        await db.query(`INSERT INTO transactions (id, store_id, total_amount, payment_method, status, transaction_date, created_at, updated_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
       ON CONFLICT(id) DO NOTHING`, [tx1, storeId1, 1000000, 'CASH', 'COMPLETED', txnDate, new Date(), new Date()]);
        // Store 1: More CASH transactions
        const tx2 = uuidv4();
        await db.query(`INSERT INTO transactions (id, store_id, total_amount, payment_method, status, transaction_date, created_at, updated_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
       ON CONFLICT(id) DO NOTHING`, [tx2, storeId1, 200000, 'CASH', 'COMPLETED', txnDate, new Date(), new Date()]);
        // Store 1: MEMBER transaction
        const tx3 = uuidv4();
        await db.query(`INSERT INTO transactions (id, store_id, total_amount, payment_method, status, transaction_date, created_at, updated_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
       ON CONFLICT(id) DO NOTHING`, [tx3, storeId1, 600000, 'MEMBER', 'COMPLETED', txnDate, new Date(), new Date()]);
        // Store 2: CASH transaction
        const tx4 = uuidv4();
        await db.query(`INSERT INTO transactions (id, store_id, total_amount, payment_method, status, transaction_date, created_at, updated_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
       ON CONFLICT(id) DO NOTHING`, [tx4, storeId2, 2000000, 'CASH', 'COMPLETED', txnDate, new Date(), new Date()]);
        // Store 2: TEMPO transaction
        const tx5 = uuidv4();
        await db.query(`INSERT INTO transactions (id, store_id, total_amount, payment_method, status, transaction_date, created_at, updated_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
       ON CONFLICT(id) DO NOTHING`, [tx5, storeId2, 200000, 'TEMPO', 'COMPLETED', txnDate, new Date(), new Date()]);
        // Store 3: Single CASH transaction
        const tx6 = uuidv4();
        await db.query(`INSERT INTO transactions (id, store_id, total_amount, payment_method, status, transaction_date, created_at, updated_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
       ON CONFLICT(id) DO NOTHING`, [tx6, storeId3, 1800000, 'CASH', 'COMPLETED', txnDate, new Date(), new Date()]);
    ...
    afterEach(async () => {
        ...
        // Cleanup stores
        await db.query(`DELETE FROM stores WHERE id IN ($1, $2, $3)`, [storeId1, storeId2, storeId3]);
    });
    // ====================================================================
    // BASIC TESTS
    // ====================================================================
    ...
    describe('GET /api/v1/reports/sales/daily - Payment method breakdown is accurate', () => {
        it('should correctly aggregate payment methods', async () => {
            const res = await request(app)
                ...
                .query({ date: testDate });
            expect(res.status).toBe(200);
            const store1 = res.body.data.byStore.find((s) => s.storeId === storeId1);
        ...
        it('should include all payment methods used on the day', async () => {
            const res = await request(app)
                ...
                .query({ date: testDate });
            expect(res.status).toBe(200);
            const store2 = res.body.data.byStore.find((s) => s.storeId === storeId2);
    ...
    describe('GET /api/v1/reports/sales/daily - Multiple stores handled correctly', () => {
        it('should include all stores with transactions', async () => {
            ...
            const storeIds = res.body.data.byStore.map((s) => s.storeId);
    ...
    describe('GET /api/v1/reports/sales/daily - Revenue calculation is correct', () => {
        it('should calculate total revenue per store correctly', async () => {
            ...
            const store3 = res.body.data.byStore.find((s) => s.storeId === storeId3);
            // Store 3: 1.8M
    ...
    describe('GET /api/v1/reports/sales/daily - Average transaction calculation', () => {
        ...
        it('should return 0 for stores with no transactions', async () => {
            ...
            if (res.body.data.byStore.length > 0) {
                res.body.data.byStore.forEach((store) => {
    ...
    describe('GET /api/v1/reports/sales/daily - Summary totals match store totals', () => {
        it('should sum all store revenue to match summary total', async () => {
            const res = await request(app)
                ...
                .query({ date: testDate });
            expect(res.status).toBe(200);
            const storeRevenues = res.body.data.byStore.reduce((sum, store) => sum + store.revenue, 0);
        ...
        it('should sum all store transactions to match summary total', async () => {
            const res = await request(app)
                ...
                .query({ date: testDate });
            expect(res.status).toBe(200);
            const totalTxns = res.body.data.byStore.reduce((sum, store) => sum + store.transactionCount, 0);
        ...
        it('should have correct store count in summary', async () => {
            const res = await request(app)
                ...
                .query({ date: testDate });
            expect(res.status).toBe(200);
            expect(res.body.data.summary.storeCount).toBe(res.body.data.byStore.length);
        });
    });
    // ====================================================================
    // EDGE CASE TESTS
    ...
    describe('GET /api/v1/reports/sales/daily - Handles decimal amounts properly', () => {
        it('should handle decimal amounts in revenue calculations', async () => {
            ...
            res.body.data.byStore.forEach((store) => {
                ...
                Object.values(store.paymentMethods).forEach((method) => {
...
describe('Weekly Sales Report Routes (Requirements 24.1-24.5)', () => {
    ...
    beforeEach(async () => {
        ...
        for (let i = 0; i < dates.length; i++) {
            ...
            await db.query(`INSERT INTO transactions (id, store_id, total_amount, payment_method, status, transaction_date, created_at, updated_at)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
         ON CONFLICT(id) DO NOTHING`, [tx, storeId1, 1000000, 'CASH', 'COMPLETED', txnDate, new Date(), new Date()]);
        }
        ...
        for (let i = 0; i < 3; i++) {
            ...
            await db.query(`INSERT INTO transactions (id, store_id, total_amount, payment_method, status, transaction_date, created_at, updated_at)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
         ON CONFLICT(id) DO NOTHING`, [tx, storeId2, 2000000, paymentMethods[i], 'COMPLETED', txnDate, new Date(), new Date()]);
        }
        // Store 3: Single transaction
        const tx = uuidv4();
        await db.query(`INSERT INTO transactions (id, store_id, total_amount, payment_method, status, transaction_date, created_at, updated_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
       ON CONFLICT(id) DO NOTHING`, [tx, storeId3, 1500000, 'CASH', 'COMPLETED', new Date('2024-01-15T10:00:00Z'), new Date(), new Date()]);
    });
    afterEach(async () => {
        await db.query(`DELETE FROM transactions WHERE store_id IN ($1, $2, $3)`, [
            storeId1,
    ...
    describe('GET /api/v1/reports/sales/weekly - Daily breakdown accuracy', () => {
        ...
        it('should show transaction data for each day', async () => {
            ...
            store1.dailyBreakdown.forEach((day) => {
    ...
    describe('GET /api/v1/reports/sales/weekly - Store filter', () => {
        ...
        it('should return empty when filtered store has no transactions', async () => {
            ...
            await db.query(`INSERT INTO stores (id, name, is_active) VALUES ($1, $2, $3)
         ON CONFLICT(id) DO NOTHING`, [emptyStoreId, 'Empty Store', true]);
            const res = await request(app)
                .get('/api/v1/reports/sales/weekly')
...
describe('Monthly Sales Report Routes (Requirements 25.1-25.6)', () => {
    ...
    beforeEach(async () => {
        ...
        for (const product of products) {
            await db.query(`INSERT INTO products (id, name, category, purchase_price, selling_price, is_active) 
         VALUES ($1, $2, $3, $4, $5, true)
         ON CONFLICT(id) DO NOTHING`, [product.id, product.name, product.category, product.purchase_price, product.selling_price]);
        }
        ...
        const txnPromises = transactionDates.map(async (date, idx) => {
            ...
            await db.query(`INSERT INTO transactions (id, store_id, total_amount, payment_method, status, transaction_date, created_at, updated_at)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
         ON CONFLICT(id) DO NOTHING`, [txnId, storeId, amount, 'CASH', 'COMPLETED', date, new Date(), new Date()]);
            ...
            for (let i = 0; i < 2; i++) {
                ...
                await db.query(`INSERT INTO transaction_items (id, transaction_id, product_id, quantity, unit_price, line_total, created_at, updated_at)
           VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
           ON CONFLICT(id) DO NOTHING`, [itemId, txnId, productIds[i], quantity, unitPrice, lineTotal, new Date(), new Date()]);
            }
        });
    ...
    afterEach(async () => {
        ...
        // Cleanup transactions
        await db.query(`DELETE FROM transactions WHERE store_id IN ($1, $2, $3)`, [storeId1, storeId2, storeId3]);
        // Cleanup products
        await db.query(`DELETE FROM products WHERE id IN ($1, $2, $3)`, [productId1, productId2, productId3]);
        // Cleanup stores
        await db.query(`DELETE FROM stores WHERE id IN ($1, $2, $3)`, [storeId1, storeId2, storeId3]);
    });
    // ====================================================================
    ...
    describe('GET /api/v1/reports/sales/monthly - Revenue calculation is correct', () => {
        ...
        it('should calculate average transaction correctly', async () => {
            const res = await request(app)
                ...
                .query({ month: testMonth, year: testYear })
                .expect(200);
            const expectedAvg = Math.floor(res.body.data.summary.totalRevenue / res.body.data.summary.totalTransactions);
