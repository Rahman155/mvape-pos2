...
router.get('/', requireAuth(), requireRole('OWNER'), async (req, res) => {
    try {
        const { page = 1, limit = 20, search = '', status = 'all', // 'all', 'active', 'inactive'
         } = req.query;
...
router.post('/', requireAuth(), requireRole('OWNER'), async (req, res) => {
    try {
        ...
        await db.query(`INSERT INTO change_history 
...
router.put('/:id', requireAuth(), requireRole('OWNER'), async (req, res) => {
    try {
        ...
        const updateOperatingHours = operatingHours !== undefined ? operatingHours : existing.operating_hours;
        const updateIsActive = isActive !== undefined ? isActive : existing.is_active;
        const now = new Date();
        const updateResult = await db.query(`UPDATE stores 
         SET name = $1, address = $2, phone = $3, operating_hours = $4, is_active = $5, updated_at = $6
         WHERE id = $7
         RETURNING *`, [updateName, updateAddress, updatePhone, updateOperatingHours, updateIsActive, now, id]);
 ...
 */
router.get('/:id/change-history', requireAuth(), requireRole('OWNER'), async (req, res) => {
    try {
        const { id } = req.params;
        ...
        // Get change history
        const historyResult = await db.query(`SELECT ch.id, ch.entity_type, ch.entity_id, ch.changed_by, ch.change_type, 
 ...
 */
router.post('/:id/check-deletion', requireAuth(), requireRole('OWNER'), async (req, res) => {
    try {
        const { id } = req.params;
        // Verify store exists
        const storeResult = await db.query('SELECT id FROM stores WHERE id = $1', [id]);
        ...
        const txnResult = await db.query('SELECT COUNT(*) as count FROM transactions WHERE store_id = $1', [id]);
        const transactionCount = Number(txnResult.rows[0].count);
        // Check for inventory
        const invResult = await db.query('SELECT COUNT(*) as count FROM inventory WHERE store_id = $1 AND quantity > 0', [id]);
        const inventoryCount = Number(invResult.rows[0].count);
        // Check for users assigned to store
        const usersResult = await db.query('SELECT COUNT(*) as count FROM users WHERE store_id = $1 AND is_active = true', [id]);
