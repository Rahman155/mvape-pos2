 ...
 * }
 */
router.get('/stores', authorize(['OWNER', 'KASIR']), async (req, res) => {
    try {
        const user = req.user;
 ...
 */
router.get('/stores/:storeId', authorize(['OWNER', 'KASIR']), authorizeStore('storeId'), async (req, res) => {
    try {
 ...
 * }
 */
router.post('/stores', authorize('OWNER'), async (req, res) => {
    try {
 ...
 * Allowed roles: OWNER only
 */
router.put('/stores/:storeId', authorize('OWNER'), async (req, res) => {
    try {
        const { storeId } = req.params;
        const { name, address, phone, operatingHours } = req.body;
 ...
 */
router.post('/transactions', authorize(['OWNER', 'KASIR']), authorizeStore('storeId'), async (req, res) => {
    try {
 ...
 */
router.get('/transactions', authorize(['OWNER', 'KASIR']), async (req, res) => {
    try {
        const user = req.user;
...
router.put('/transactions/:transactionId', authorize(['OWNER', 'KASIR']), authorizeResourceOwnership(checkTransactionOwnership), async (req, res) => {
    try {
        const { transactionId } = req.params;
 ...
 */
router.get('/reports/daily', authorize('OWNER'), async (req, res) => {
    try {
 ...
 */
router.get('/reports/bop', authorize(['OWNER', 'KASIR']), async (req, res) => {
    try {
        const user = req.user;
        let storeId = req.query.storeId;
        ...
        logger.error('Get BOP report error', error);
        res.status(500).json({ error: 'Internal Server Error' });
    }
});
