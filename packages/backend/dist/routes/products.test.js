...
describe('Product Routes', () => {
    ...
    describe('GET /products/search', () => {
        it('should search products by name', async () => {
            ...
            const response = await app._request('GET', '/products/search?q=juice&storeId=store-123&limit=20&offset=0');
        ...
        it('should search products by SKU', async () => {
            ...
            redis.get.mockResolvedValueOnce(null);
            redis.set.mockResolvedValueOnce('OK');
            const response = await app._request('GET', '/products/search?q=VJ-30ML-001&storeId=store-123');
            expect(response.status).toBe(200);
            expect(response.body.data).toHaveLength(1);
        ...
        it('should filter products by category', async () => {
            ...
            redis.get.mockResolvedValueOnce(null);
            redis.set.mockResolvedValueOnce('OK');
            const response = await app._request('GET', '/products/search?category=Juice&storeId=store-123');
            expect(response.status).toBe(200);
            expect(response.body.data).toHaveLength(1);
        ...
        it('should include stock availability flag', async () => {
            ...
            redis.get.mockResolvedValueOnce(null);
            redis.set.mockResolvedValueOnce('OK');
            const response = await app._request('GET', '/products/search?storeId=store-123');
            expect(response.status).toBe(200);
        ...
        it('should respect pagination limits', async () => {
            ...
            redis.get.mockResolvedValueOnce(null);
            redis.set.mockResolvedValueOnce('OK');
            const response = await app._request('GET', '/products/search?storeId=store-123&limit=10&offset=0');
            expect(response.status).toBe(200);
        ...
        it('should cache results for 5 minutes', async () => {
            ...
            redis.get.mockResolvedValueOnce(null);
            redis.set.mockResolvedValueOnce('OK');
            const response = await app._request('GET', '/products/search?q=cache&storeId=store-123');
            expect(response.status).toBe(200);
            expect(redis.set).toHaveBeenCalledWith(expect.stringContaining('products:search:'), expect.any(String), 300);
        });
    ...
    describe('GET /products/:id', () => {
        it('should retrieve a single product with inventory', async () => {
            ...
            const response = await app._request('GET', '/products/prod-1?storeId=store-123');
            expect(response.status).toBe(200);
        ...
        it('should return 404 if product not found', async () => {
            ...
            const response = await app._request('GET', '/products/nonexistent?storeId=store-123');
    ...
    describe('GET /products', () => {
        it('should list all products for a store', async () => {
            ...
            db.query
                .mockResolvedValueOnce({ rows: [{ total: 1 }] })
                .mockResolvedValueOnce({ rows: mockProducts });
            const response = await app._request('GET', '/products?storeId=store-123&limit=20&offset=0');
            expect(response.status).toBe(200);
            expect(response.body.data).toHaveLength(1);
            expect(response.body.pagination.total).toBe(1);
        });
        it('should return empty array when no products exist', async () => {
            ...
            const response = await app._request('GET', '/products?storeId=store-123');
            expect(response.status).toBe(200);
