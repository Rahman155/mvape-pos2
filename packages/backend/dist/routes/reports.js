 ...
 * }
 */
reportsRouter.get('/sales/daily', authorize('OWNER'), async (req, res, next) => {
    try {
        ...
        if (dateParam) {
            ...
            if (!dateRegex.test(dateParam)) {
                throw new ApiError('Invalid date format. Use YYYY-MM-DD', ApiErrorCode.BAD_REQUEST, 400);
            }
        ...
        if (targetDate > today) {
            throw new ApiError('Cannot query future dates', ApiErrorCode.BAD_REQUEST, 400);
        }
        // Format date for cache key and display
        const dateString = targetDate.toISOString().split('T')[0];
        // Try to get from cache
        const cacheKey = `report:sales:daily:${dateString}`;
        let cachedReport = await CacheService.get(cacheKey);
        if (cachedReport) {
        ...
        const byStore = Array.from(storeMap.values()).map((store) => ({
            ...store,
            averageTransaction: store.transactionCount > 0
 ...
 */
function getDayOfWeekIndonesian(date) {
...
reportsRouter.get('/sales/weekly', authorize('OWNER'), async (req, res, next) => {
    try {
        ...
        if (!week) {
            throw new ApiError('Week parameter is required (1-53)', ApiErrorCode.BAD_REQUEST, 400);
        }
        const weekNum = parseInt(week, 10);
        if (isNaN(weekNum) || weekNum < 1 || weekNum > 53) {
            throw new ApiError('Invalid week number. Must be between 1 and 53', ApiErrorCode.BAD_REQUEST, 400);
        }
        // Validate year parameter
        if (!year) {
            throw new ApiError('Year parameter is required (YYYY format)', ApiErrorCode.BAD_REQUEST, 400);
        }
        const yearNum = parseInt(year, 10);
        const currentYear = new Date().getFullYear();
        if (isNaN(yearNum) || yearNum < currentYear - 2 || yearNum > currentYear + 2) {
            throw new ApiError(`Year must be between ${currentYear - 2} and ${currentYear + 2}`, ApiErrorCode.BAD_REQUEST, 400);
        }
        // Check for future weeks
        const { start: weekStart, end: weekEnd } = getISOWeekDates(weekNum, yearNum);
        const now = new Date();
        if (weekStart > now) {
            throw new ApiError('Cannot query future weeks', ApiErrorCode.BAD_REQUEST, 400);
        }
      ...
      `;
        ...
        result.rows.forEach((row) => {
            ...
            if (row.transaction_id && row.total_amount && row.payment_method) {
                ...
                const dailyEntry = store.dailyBreakdown.find((d) => d.date === txnDateStr);
                if (dailyEntry) {
...
reportsRouter.get('/sales/monthly', authorize('OWNER'), async (req, res, next) => {
    try {
        ...
        if (!monthParam) {
            throw new ApiError('Month parameter is required (1-12)', ApiErrorCode.BAD_REQUEST, 400);
        }
        const month = parseInt(monthParam, 10);
        if (isNaN(month) || month < 1 || month > 12) {
            throw new ApiError('Invalid month number. Must be between 1 and 12', ApiErrorCode.BAD_REQUEST, 400);
        }
        // Validate year parameter
        if (!yearParam) {
            throw new ApiError('Year parameter is required (YYYY format)', ApiErrorCode.BAD_REQUEST, 400);
        ...
        const monthStart = new Date(Date.UTC(year, month - 1, 1));
        if (monthStart > now) {
            throw new ApiError('Cannot query future months', ApiErrorCode.BAD_REQUEST, 400);
        }
        // Format dates for display
      ...
      `;
        ...
        result.rows.forEach((row) => {
            ...
            if (row.transaction_id && row.total_amount && row.payment_method) {
                ...
                if (!store.processedTransactions) {
                    store.processedTransactions = new Set();
                }
                if (!store.processedTransactions.has(row.transaction_id)) {
                    ...
                    const weekEntry = store.weeklyBreakdown.find((w) => txnDate >= new Date(w.weekStart) && txnDate <= new Date(w.weekEnd + 'T23:59:59Z'));
                    if (weekEntry) {
        ...
        const topProducts = Array.from(productMap.values())
            ...
            averagePrice: product.quantitySold > 0 ? Math.floor(product.revenue / product.quantitySold) : 0,
        }));
        ...
        const reportData = {
            ...
            summary: {
                ...
                totalTransactions: summaryTransactions,
                storeCount: byStore.length,
                averageTransaction: summaryTransactions > 0
 ...
 */
reportsRouter.get('/financial/profit-loss', authorize('OWNER'), async (req, res, next) => {
    try {
        const monthParam = req.query.month;
        const yearParam = req.query.year;
        const storeIdParam = req.query.storeId;
 ...
 */
reportsRouter.get('/financial/inventory-valuation', authorize('OWNER'), async (req, res, next) => {
    try {
        const dateParam = req.query.date;
        // Validate date format if provided
        if (dateParam) {
 ...
 */
reportsRouter.get('/financial/cash-flow', authorize('OWNER'), async (req, res, next) => {
    try {
        const monthParam = req.query.month;
        const yearParam = req.query.year;
        const storeIdParam = req.query.storeId;
      ...
      `;
        ...
        const result = await db.query(query, params);
        // Process results
        const byStore = result.rows.map((row) => {
 ...
 * Requirements: 17.1, 17.2, 17.3
 */
reportsRouter.get('/bop', authorize('OWNER'), async (req, res, next) => {
    try {
        const periodParam = req.query.period;
        const storeIdParam = req.query.storeId;
        // Validate period parameter
        if (!periodParam || !['daily', 'weekly', 'monthly'].includes(periodParam)) {
            throw new ApiError("Period parameter is required and must be 'daily', 'weekly', or 'monthly'", ApiErrorCode.BAD_REQUEST, 400);
        }
 ...
 */
async function handleBOPDailyReport(req, res, next, storeIdParam) {
    try {
        const dateParam = req.query.date;
        if (!dateParam) {
            throw new ApiError('Date parameter is required for daily report (YYYY-MM-DD)', ApiErrorCode.BAD_REQUEST, 400);
        }
        const dateRegex = /^\d{4}-\d{2}-\d{2}$/;
        if (!dateRegex.test(dateParam)) {
            throw new ApiError('Invalid date format. Use YYYY-MM-DD', ApiErrorCode.BAD_REQUEST, 400);
 ...
 */
async function handleBOPWeeklyReport(req, res, next, storeIdParam) {
    try {
        ...
        const weekEndStr = weekEnd.toISOString().split('T')[0];
        // Try to get from cache
        const cacheKey = `report:bop:weekly:${yearNum}-W${String(weekNum).padStart(2, '0')}${storeIdParam ? `:${storeIdParam}` : ''}`;
        let cachedReport = await CacheService.get(cacheKey);
        if (cachedReport) {
      ...
      WHERE b.effective_from <= $2
        ...
        AND s.is_active = true
    `;
        const params = [weekStartStr, weekEndStr];
        if (storeIdParam) {
            query += ` AND b.store_id = $${params.length + 1}`;
            params.push(storeIdParam);
        }
 ...
 */
async function handleBOPMonthlyReport(req, res, next, storeIdParam) {
    try {
        const monthParam = req.query.month;
        const yearParam = req.query.year;
        if (!monthParam) {
        ...
        const monthEndStr = monthEnd.toISOString().split('T')[0];
        // Try to get from cache
        const cacheKey = `report:bop:monthly:${year}-${String(month).padStart(2, '0')}${storeIdParam ? `:${storeIdParam}` : ''}`;
        let cachedReport = await CacheService.get(cacheKey);
        if (cachedReport) {
      ...
      WHERE b.effective_from <= $2
        ...
        AND s.is_active = true
    `;
        const params = [monthStartStr, monthEndStr];
        if (storeIdParam) {
            query += ` AND b.store_id = $${params.length + 1}`;
            params.push(storeIdParam);
        }
 ...
 */
reportsRouter.get('/bop/export/pdf', authorize('OWNER'), async (req, res, next) => {
    try {
        ...
        if (period === 'daily') {
            ...
            reportData = await CacheService.get(cacheKey);
            if (!reportData) {
            ...
            WHERE b.effective_from <= $1
              ...
              AND s.is_active = true
            ${storeId ? 'AND s.id = $3' : ''}
            ORDER BY s.name, b.name`, storeId ? [todayEnd, todayStart, storeId] : [todayEnd, todayStart]);
                const storeMap = new Map();
                let summaryBOP = 0;
                result.rows.forEach((row) => {
                    if (!storeMap.has(row.store_id)) {
            ...
            filename = `BOP_Report_Daily_${date || new Date().toISOString().split('T')[0]}.pdf`;
        }
            ...
            WHERE b.effective_from <= $1
              ...
              AND s.is_active = true
            ${storeId ? 'AND s.id = $3' : ''}
            ORDER BY s.name, b.name`, storeId ? [weekEnd, weekStart, storeId] : [weekEnd, weekStart]);
                const storeMap = new Map();
                let summaryBOP = 0;
                result.rows.forEach((row) => {
                    if (!storeMap.has(row.store_id)) {
            ...
            WHERE b.effective_from <= $1
              ...
              AND s.is_active = true
            ${storeId ? 'AND s.id = $3' : ''}
            ORDER BY s.name, b.name`, storeId ? [monthEnd, monthStart, storeId] : [monthEnd, monthStart]);
                const storeMap = new Map();
                let summaryBOP = 0;
                result.rows.forEach((row) => {
                    if (!storeMap.has(row.store_id)) {
        ...
        logger.error('Error exporting BOP report to PDF:', error);
        next(error);
    }
});
/**
 ...
 */
reportsRouter.get('/bop/export/excel', authorize('OWNER'), async (req, res, next) => {
    try {
        const { period, month, year, week, date, storeId } = req.query;
        // Validate period
        if (!period || !['daily', 'weekly', 'monthly'].includes(period)) {
        ...
        if (period === 'daily') {
            ...
            filename = `BOP_Report_Daily_${date || new Date().toISOString().split('T')[0]}.csv`;
        }
        else if (period === 'weekly') {
            const weekNum = parseInt(week) || 1;
            const yearNum = parseInt(year) || new Date().getFullYear();
        ...
        logger.error('Error exporting BOP report to Excel:', error);
        next(error);
    }
});
