 ...
 * }
 */
dashboardRouter.get('/kasir/daily-stats', async (req, res, next) => {
    try {
        // Verify user is authenticated and has appropriate role
        if (!req.user || (req.user.role !== 'KASIR' && req.user.role !== 'OWNER')) {
            throw new ApiError('Kasir or Owner role required', ApiErrorCode.FORBIDDEN, 403);
        }
 ...
 * }
 */
dashboardRouter.get('/owner/summary', async (req, res, next) => {
    try {
        // Verify user is authenticated and is owner
        if (!req.user || req.user.role !== 'OWNER') {
            throw new ApiError('Owner role required', ApiErrorCode.FORBIDDEN, 403);
        }
        // Get today's stats
        ...
        // Query store count
        const storeCountResult = await db.query(`SELECT COUNT(*) as count FROM stores WHERE is_active = true`);
