...
router.post('/', authorize('OWNER'), async (req, res) => {
    try {
        ...
        const existingSupplier = await db.query('SELECT id FROM suppliers WHERE LOWER(name) = LOWER($1)', [name.trim()]);
        ...
        const now = new Date();
        await db.query(`INSERT INTO suppliers (id, name, phone, email, address, payment_terms, is_active, created_at, updated_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`, [
        ...
        const supplierResult = await db.query(`SELECT 
...
router.put('/:id', authorize('OWNER'), async (req, res) => {
    try {
        ...
        const supplierResult = await db.query('SELECT id FROM suppliers WHERE id = $1', [id]);
        ...
        if (name !== undefined) {
            ...
            const nameExists = await db.query('SELECT id FROM suppliers WHERE LOWER(name) = LOWER($1) AND id != $2', [name.trim(), id]);
...
router.delete('/:id', authorize('OWNER'), async (req, res) => {
    try {
        const { id } = req.params;
        // Verify supplier exists
        const supplierResult = await db.query('SELECT id, name FROM suppliers WHERE id = $1', [id]);
        if (supplierResult.rows.length === 0) {
        ...
        const poResult = await db.query(`SELECT COUNT(*) as count FROM purchase_orders 
       WHERE supplier_id = $1 AND status IN ('PENDING', 'RECEIVED')`, [id]);
