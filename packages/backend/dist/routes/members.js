...
router.get('/:id', requireAuth(), async (req, res) => {
    try {
        ...
        const transactionsResult = await db.query(`SELECT t.id, t.total_amount, t.payment_method, t.transaction_date, t.status
       FROM transactions t
...
router.post('/', requireAuth(), async (req, res) => {
    try {
       ...
       RETURNING *`, [id, memberNumber, name.trim(), phone || null, email || null, '0', '0', true, now, now]);
 ...
 */
router.put('/:id/credit', requireAuth(), async (req, res) => {
    try {
        const { id } = req.params;
        const { amount, type } = req.body;
        // Check authorization - only owner can topup
        if (type === 'TOPUP' && req.user.role !== 'OWNER') {
            return res
                .status(403)
        ...
        const now = new Date();
        const updateResult = await db.query('UPDATE members SET credit_balance = $1, updated_at = $2 WHERE id = $3 RETURNING *', [newBalance.toString(), now, id]);
...
router.post('/:id/topup', requireAuth(), async (req, res) => {
    try {
        ...
        await db.query(`INSERT INTO credit_transactions 
