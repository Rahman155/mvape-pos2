...
router.post('/initiate', requireAuth(), async (req, res) => {
    ...
    try {
        ...
        const opnameResult = await client.query(`INSERT INTO stock_opnames (store_id, conducted_by, status, opname_date)
       VALUES ($1, $2, $3, NOW())
       RETURNING id, store_id, opname_date, status, conducted_by`, [storeId, userId, 'ONGOING']);
       ...
       ORDER BY p.name ASC`, [storeId]);
        ...
        if (products.length > 0) {
            await client.query(`INSERT INTO opname_details (opname_id, product_id, system_quantity, physical_quantity, status, notes)
...
router.post('/:sessionId/items', requireAuth(), async (req, res) => {
    ...
    try {
        ...
        const opnameCheck = await client.query('SELECT id, status FROM stock_opnames WHERE id = $1', [sessionId]);
        ...
        for (const item of items) {
            ...
            if (typeof physicalQuantity !== 'number' ||
                physicalQuantity < 0 ||
            ...
            const detailResult = await client.query(`SELECT id, system_quantity, physical_quantity 
           FROM opname_details 
           WHERE opname_id = $1 AND product_id = $2`, [sessionId, productId]);
            ...
            const updateResult = await client.query(`UPDATE opname_details
           SET physical_quantity = $1, difference = $2, status = $3
           WHERE opname_id = $4 AND product_id = $5
           RETURNING id, system_quantity, physical_quantity, difference, status`, [physicalQuantity, difference, status, sessionId, productId]);
...
router.get('/:sessionId', requireAuth(), async (req, res) => {
    ...
    try {
        ...
        const opnameResult = await client.query(`SELECT 
...
router.post('/:sessionId/complete', requireAuth(), async (req, res) => {
    ...
    try {
        ...
        // Fetch opname session
        const opnameResult = await client.query('SELECT id, store_id, status FROM stock_opnames WHERE id = $1', [sessionId]);
        if (opnameResult.rows.length === 0) {
            return res.status(404).json({
                error: 'Opname session not found',
                code: 'NOT_FOUND',
        ...
        try {
            ...
            for (const item of items) {
             ...
             WHERE product_id = $2 AND store_id = $3`, [item.physical_quantity, item.product_id, opnameSession.store_id]);
            }
            // Update opname session status
            const updatedOpname = await client.query(`UPDATE stock_opnames 
           SET status = $1, verified_by = $2, updated_at = NOW()
           WHERE id = $3
           RETURNING id, store_id, status`, ['VERIFIED', userId, sessionId]);
    ...
    try {
        ...
        const totals = {
            ...
            totalExcessValue: items
                ...
                .reduce((sum, i) => sum + (i.financial_impact || 0), 0),
            netFinancialImpact: items.reduce((sum, i) => sum + (i.financial_impact || 0), 0),
        };
        res.status(200).json({
            success: true,
            data: {
...
//# sourceMappingURL=stock-opname.js.map