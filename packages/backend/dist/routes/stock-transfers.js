...
router.post('/', requireAuth(), async (req, res) => {
    try {
        ...
        const warehouseResult = await db.query("SELECT id FROM stores WHERE name = 'WAREHOUSE' OR name LIKE '%Warehouse%' LIMIT 1");
        ...
        for (const item of items) {
            ...
            const warehouseInventoryResult = await db.query('SELECT quantity FROM inventory WHERE product_id = $1 AND store_id = $2', [productId, warehouseId]);
        ...
        const now = new Date();
        const transferResult = await db.query(`INSERT INTO stock_transfers 
       ...
       RETURNING *`, [
        ...
        for (const item of items) {
            ...
            const itemResult = await db.query(`INSERT INTO stock_transfer_items 
         (id, stock_transfer_id, product_id, quantity, received_quantity, created_at)
         VALUES ($1, $2, $3, $4, $5, $6)
         RETURNING *`, [itemId, transferId, productId, quantity, quantity, now]);
            // Get warehouse inventory before transfer
            const warehouseInvResult = await db.query('SELECT * FROM inventory WHERE product_id = $1 AND store_id = $2', [productId, warehouseId]);
            const warehouseInv = warehouseInvResult.rows[0];
            totalBeforeTransfer += warehouseInv.quantity;
            // Deduct from warehouse
            await db.query('UPDATE inventory SET quantity = quantity - $1, updated_at = $2 WHERE product_id = $3 AND store_id = $4', [quantity, now, productId, warehouseId]);
            // Get or create destination store inventory
            const destInvResult = await db.query('SELECT * FROM inventory WHERE product_id = $1 AND store_id = $2', [productId, destinationStoreId]);
           ...
           VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`, [uuidv4(), productId, destinationStoreId, quantity, 0, 10, now, now]);
            }
            else {
                // Add to destination store
                await db.query('UPDATE inventory SET quantity = quantity + $1, updated_at = $2 WHERE product_id = $3 AND store_id = $4', [quantity, now, productId, destinationStoreId]);
            }
            // Get warehouse inventory after transfer
            const warehouseInvAfter = await db.query('SELECT quantity FROM inventory WHERE product_id = $1 AND store_id = $2', [productId, warehouseId]);
        ...
        const warehouseTotalBefore = await db.query(`SELECT COALESCE(SUM(quantity), 0) as total FROM inventory WHERE store_id = $1`, [warehouseId]);
        const warehouseTotalAfter = await db.query(`SELECT COALESCE(SUM(quantity), 0) as total FROM inventory WHERE store_id = $1`, [warehouseId]);
...
router.get('/', requireAuth(), async (req, res) => {
    try {
        ...
        let query = 'SELECT st.*, s.name as to_store_name FROM stock_transfers st JOIN stores s ON st.to_store_id = s.id';
        const params = [];
        if (status) {
...
router.get('/:id', requireAuth(), async (req, res) => {
    try {
        const { id } = req.params;
        // Get transfer
        const transferResult = await db.query('SELECT * FROM stock_transfers WHERE id = $1', [id]);
        ...
        const itemsResult = await db.query(`SELECT sti.*, p.name as product_name, p.sku, p.category, p.cost_price, p.selling_price
       FROM stock_transfer_items sti
       JOIN products p ON sti.product_id = p.id
       WHERE sti.stock_transfer_id = $1`, [id]);
