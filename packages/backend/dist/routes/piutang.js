...
router.get('/alerts/upcoming', authorize('OWNER'), async (req, res) => {
    try {
       ...
       ORDER BY p.due_date ASC`, [todayStr, sevenDaysStr]);
        const upcomingPiutang = result.rows.map((row) => ({
            id: row.id,
        ...
        logger.error('Error fetching upcoming piutang:', error, {
...
router.get('/alerts/overdue', authorize('OWNER'), async (req, res) => {
    try {
       ...
       ORDER BY p.due_date ASC`, [today]);
        const overduePiutang = result.rows.map((row) => ({
            id: row.id,
            transactionId: row.transaction_id,
            memberId: row.member_id,
            customerName: row.customer_name,
        ...
        logger.error('Error fetching overdue piutang:', error, {
            requestId: req.requestId,
            userId: req.user?.id,
        });
...
router.get('/', authorize('OWNER'), async (req, res) => {
    try {
        const { page = 1, limit = 20, status, customerName, dueDateFrom, dueDateTo, amountMin, amountMax, sort = 'due_date', } = req.query;
    ...
    `;
        ...
        const piutang = result.rows.map((row) => ({
            id: row.id,
            transactionId: row.transaction_id,
            memberId: row.member_id,
            customerName: row.customer_name,
        ...
        logger.error('Error fetching piutang list:', error, {
            requestId: req.requestId,
            userId: req.user?.id,
        });
    ...
    try {
       ...
       WHERE p.id = $1`, [id]);
        ...
        if (row.transaction_id) {
            const transResult = await db.query(`SELECT 
          t.id,
         ...
         LIMIT 10`, [memberId]);
            transactionHistory = transResult.rows.map((t) => ({
                id: t.id,
 ...
 */
router.post('/:id/payment', authorize('OWNER'), async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const piutangResult = await db.query(`SELECT 
        id, 
        amount, 
        remaining_balance, 
        status, 
